...
/** @type {{ weekday: number, enabled: boolean, open: string, close: string, closedReason?: string }[] | null} */
let memoryBusinessHours = null;

function normalizeClosedReason(raw) {
  return String(raw ?? "")
    .replace(/\r?\n/g, " ")
    .replace(/\s+/g, " ")
    .trim()
...
/**
 * @param {unknown} body
 * @returns {{ ok: true, days: { weekday: number, enabled: boolean, open: string, close: string, closedReason: string }[] } | { ok: false, error: string }}
 */
export function parseAndValidateDays(body) {
  ...
  for (let i = 0; i < 7; i++) {
    const row = raw[i];
    const enabled = coerceDayEnabled(row?.enabled);
    ...
    out.push({
      ...
      open,
      close,
      closedReason: normalizeClosedReason(row?.closedReason),
    });
  }
  return { ok: true, days: out };
}

/** @returns {Promise<{ weekday: number, enabled: boolean, open: string, close: string, closedReason: string }[]>} */
...
/**
 * @param {{ weekday: number, enabled: boolean, open: string, close: string, closedReason?: string }[]} days
 */
  ...
  try {
    ...
    memoryBusinessHours = Array.isArray(days)
      ? days.map((d, i) => ({
          ...
          enabled: coerceDayEnabled(d?.enabled),
          open: String(d?.open ?? ""),
          close: String(d?.close ?? ""),
          closedReason: normalizeClosedReason(d?.closedReason),
        }))
      : null;
    return false;
  }
