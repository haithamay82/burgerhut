...
/**
 * נתיב סטטי תחת public/home-slider/ — כמו תמונות /menu/ (אותו דומיין, תמיד זמין ללקוחות).
 */
export function isSliderPublicPath(u) {
  const s = String(u || "").trim();
  if (!s.startsWith("/home-slider/")) return false;
  if (s.includes("..") || s.includes("//") || s.includes("?") || s.includes("#")) {
    return false;
  }
  if (s.length < 15 || s.length > 240) return false;
  if (
    !/^\/home-slider\/[a-zA-Z0-9][a-zA-Z0-9_.-]*\.(jpe?g|png|gif|webp|avif)$/i.test(
      s
    )
  ) {
    return false;
...
function isAllowedSliderUrl(url) {
  const u = String(url || "").trim();
  return /^https?:\/\//i.test(u) || isSliderPublicPath(u);
}
...
function normalizeSliderShape(raw) {
  ...
  for (const row of raw.images || []) {
    ...
    if (!id || !url || !isAllowedSliderUrl(url)) continue;
    images.push({ id, url });
  }
  return {
