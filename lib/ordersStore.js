...
/** ברירת מחדל 5000; לספירה מ־1 (כמו בווטסאפ) הגדירו ORDER_FIRST_NUMBER=1 ב־Vercel */
const ORDER_FIRST_NUMBER = (() => {
  const n = Number(process.env.ORDER_FIRST_NUMBER);
...
function getNextOrderNumber(orders) {
  if (!Array.isArray(orders) || !orders.length) return ORDER_FIRST_NUMBER;
  let maxOrderNumber = ORDER_FIRST_NUMBER - 1;
  ...
  return Math.max(ORDER_FIRST_NUMBER, maxOrderNumber + 1);
}
...
export async function appendOrder(payload) {
  ...
  if (Array.isArray(fromKv.orders)) {
    ...
    const nextOrderNumber = Math.max(
      nextByData,
      Number(fromKv.nextOrderNumber) || ORDER_FIRST_NUMBER
    );
    const row = {
      id,
