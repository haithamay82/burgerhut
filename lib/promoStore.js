...
const KV_KEY = "burgerhut:promo-meta";

const ALLOWED_EXT = new Set([".mp4", ".webm", ".mov"]);

/**
 * @returns {Promise<{ enabled: boolean, filename: string | null, externalUrl: string | null, updatedAt: number }>}
 */
export async function getPromoMeta() {
  const fromKv = await kvGetJson(KV_KEY);
  if (fromKv && typeof fromKv === "object") {
    const fn =
      typeof fromKv.filename === "string" && fromKv.filename.startsWith("promo.")
        ? fromKv.filename
        : null;
    const externalUrl =
      typeof fromKv.externalUrl === "string" &&
      /^https?:\/\//i.test(fromKv.externalUrl)
        ? fromKv.externalUrl
        : null;
    return {
  ...
  try {
    ...
    return { enabled: false, filename: null, externalUrl: null, updatedAt: 0 };
  }
}

/**
 * @param {Partial<{ enabled: boolean, filename: string | null, externalUrl: string | null, updatedAt: number }>} patch
 */
...
export async function getPromoPublicState() {
  ...
  const hasLocalFile = meta.filename && (await fileExistsInUploads(meta.filename));
  const hasExternalFile = Boolean(meta.externalUrl);
  const hasFile = Boolean(hasExternalFile || hasLocalFile);
  const active = Boolean(meta.enabled && hasFile);
  const videoUrl = active
    ? meta.externalUrl || (meta.filename ? `/uploads/${meta.filename}` : null)
    : null;
  return {
    videoUrl,
