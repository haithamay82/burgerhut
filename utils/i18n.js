/** @typedef {'he' | 'ar'} Locale */

const HE = {
  "header.tagline":
    "מסעדת בורגר וקריספי משלוחים ואיסוף עצמי ירכא והסביבה",
  "header.goHome": "מעבר למסך הבית",
  "header.brandSecondary": "בורגר האט",
  "header.admin": "ניהול",
  "lang.switch": "שפה",
  "lang.he": "עברית",
  "lang.ar": "العربية",

  "home.title": "רעבים? הזמינו עכשיו ונכין לכם באהבה ❤",
  "home.titleLine1": "רעבים? הזמינו עכשיו",
  "home.titleLine2": "ונכין לכם באהבה",
  "home.burgerMascotAlt": "דמות מצחיקה אוכלת המבורגר",
  "home.contactTitle": "יצירת קשר ומיקום",
  "home.contactCopyright": "© אקסטרים טכנולוגיות",
  "home.contactCopyrightEmail": "extremetechnologies22@gmail.com",
  "home.contactPhoneLabel": "טלפון",
  "home.contactAddressLabel": "כתובת",
  "home.contactPhoneValue": "0504847599",
  "home.callPhoneAria": "התקשר למסעדה",
  "home.contactAddressValue": "ירכא 137",
  "home.instagramAria": "אינסטגרם של המסעדה",
  "home.tiktokAria": "טיקטוק של המסעדה",
  "home.instagramUrl": "https://www.instagram.com/burger._.hut?igsh=c3UwZzliZDBtNXQz",
  "home.tiktokUrl": "https://www.tiktok.com/@burger._.hut?_r=1&_t=ZS-95A2cZnTFdV",
  "home.openMap": "פתיחה במפות",
  "home.openWaze": "waze למסעדה",
  /** טקסט מוסתר לקוראי מסך בלבד — ללא מילה «ניהול» בממשק הגלוי */
  "home.adminDiscreetSr": "כניסת פאנל המסעדה",
  "home.termsTitle": "תקנון",
  "home.termsLead":
    "מדיניות ביטול עסקה הכפופה לחוק הגנת הצרכן הישראלי.",
  "home.termsLawCitation":
    "ביטול עסקה בהתאם לתקנות הגנת הצרכן (ביטול עסקה), התשע\"א-2010 וחוק הגנת הצרכן, התשמ\"א-1981",
  "home.hoursButton": "זמני עבודה",
  "home.restaurantOpen": "המסעדה פתוחה",
  "home.restaurantClosed": "המסעדה סגורה",
  "home.restaurantOpensAt16": "תיפתח בשעה 16:00",
  "home.hoursDialogTitle": "זמני עבודה",
  "home.hoursDays": "מיום שני עד שבת",
  "home.hoursTime": "משעה 10:00 עד שעת הסגירה (לפי היום — ראו ברשימה)",
  "home.hoursOrderWindow":
    "ניתן לשלוח הזמנות החל משעה 10:00 תחילת העבודה לפי זמני העבודה",
  "home.hoursNoOpenDays": "לא הוגדרו ימי פעילות.",
  "home.orderingClosedBannerOpenDay":
    "המסעדה תיפתח בשעה 16:00 — ניתן לעשות הזמנה משעה 10:00 עד שעת הסגירה באותו יום.",
  "home.preOrderBanner":
    "אפשר לשלוח הזמנות עכשיו. תחילת עבודה שעה {openTime}",
  "home.preOpenDialogTitle": "שימו לב",
  "home.preOpenDialogLine1":
    "המסעדה סגורה כעת, תיפתח בשעה {openTime}.",
  "home.preOpenDialogLine2":
    "ניתן לבצע הזמנה. תחילת העבודה שעה {openTime}.",
  "home.preOpenDialogAck": "הבנתי",
  "home.orderingClosedBannerDayOff": "המסעדה סגורה ב{day}",
  "home.discountAnnounceWithReason":
    "עקב {reason} מחכה לכם הנחה של {percent}% בקניה מעל {min}.",
  "home.discountAnnounceNoReason":
    "מחכה לכם הנחה של {percent}% בקניה מעל {min}.",
  "home.categoryBannerAria": "בחירת קטגוריית תפריט",
  "home.promoSkip": "דלג",
  "home.promoTapForSound": "לחץ להפעלת קול",
  "home.promoAria": "פרסום וידאו בדף הבית",
  "pwa.iosInAppBrowserTitle": "פתיחה ב־Safari או Chrome",
  "pwa.iosInAppShortLine1": "כדי להוריד את האפליקציה לאייפון שלך,",
  "pwa.iosInAppShortPrefix": "כנס לאתר באמצעות דפדפן ",
  "pwa.iosInAppShortOr": " או ",
  "pwa.iosInAppShortSuffix": ".",
  "pwa.iosInAppShortHint":
    "אחרי שנכנסתם לדפדפן: שיתוף (חץ למעלה) ← «הוסף למסך הבית».",
  "pwa.iosOpenInSafariAria": "פתיחת האתר בדפדפן Safari",
  "pwa.iosOpenInChromeAria": "פתיחת האתר בדפדפן Chrome",
  "pwa.iosCopyLinkButton": "העתק כתובת האתר",
  "pwa.iosCopyLinkDone":
    "הועתק. פתחו Safari, הדביקו בשורת הכתובת, ואז שיתוף ← הוסף למסך הבית.",
  "pwa.iosCopyLinkFail": "העתקה אוטומטית נכשלה. העתיקו ידנית:",
  "pwa.iosSafariGuideTitle": "הוספה מ-Safari",
  "pwa.iosGuideClose": "הבנתי",
  "pwa.iosInstallHint":
    "ב-Safari באייפון:\n\n• לחץ שיתוף (החץ למעלה מהריבוע).\n• גלול למטה בגיליון — «הוסף למסך הבית» לעיתים מתחת לאנשי קשר, לא בשורה העליונה.\n• אם לא מופיע: «עריכת פעולות…» בתחתית והפעל «הוסף למסך הבית».",
  "pwa.androidInstallHint":
    "אם לא נפתח חלון התקנה: לחץ על ⋮ בתפריט Chrome ובחר «התקן אפליקציה» או «הוסף למסך הבית».",
  "pwa.launcherAria": "הוספה למסך הבית — פתיחה מהירה",
  "pwa.launcherCaption": "הורד אפליקציה",

  "customerPush.bannerTitle": "עדכונים, מבצעים והנחות",
  "customerPush.bannerBody":
    "קבלו התראות על מבצעים, הנחות ומוצרים חדשים — רק אם התקנתם את Burger Hut במסך הבית.",
  "customerPush.enableBtn": "כן, שלחו לי עדכונים",
  "customerPush.dismissBtn": "לא עכשיו",
  "customerPush.enabling": "מפעיל…",
  "customerPush.permissionDenied": "התראות חסומות. אפשרו בהגדרות הדפדפן/האפליקציה.",
  "customerPush.unavailable": "התראות עדיין לא זמינות בשרת.",
  "customerPush.subscribeErr": "לא ניתן להירשם כרגע. נסו שוב מאוחר יותר.",
  "pwaNotify.title": "קבלו עדכונים על מבצעים והנחות",
  "pwaNotify.body":
    "התקנתם את Burger Hut — אפשרו התראות כדי לקבל מבצעים, הנחות וחדשות מהמסעדה.",
  "pwaNotify.systemHint":
    "אחרי «אפשר התראות» יופיע חלון של הטלפון — בחרו «אפשר».",
  "pwaNotify.enableBtn": "אפשר התראות",
  "pwaNotify.laterBtn": "אולי אחר כך",
  "pwaNotify.enabling": "מפעיל…",
  "pwaNotify.permissionDenied":
    "התראות חסומות. אפשרו בהגדרות האפליקציה/הדפדפן כדי לקבל עדכונים.",
  "pwaNotify.unavailable": "התראות עדיין לא זמינות בשרת.",
  "pwaNotify.subscribeErr": "לא ניתן להירשם כרגע. נסו שוב.",
  "rating.title": "איך הייתה החוויה?",
  "rating.subtitle": "דרגו את איכות האוכל במסעדה (1–5 כוכבים)",
  "rating.starsAria": "דירוג כוכבים",
  "rating.starN": "{n} כוכבים",
  "rating.commentPh": "הערה (אופציונלי)",
  "rating.submitBtn": "שלח דירוג",
  "rating.submitting": "שולח…",
  "rating.skipBtn": "לא עכשיו",
  "rating.thanks": "תודה על הדירוג!",
  "rating.pickStars": "בחרו מספר כוכבים",
  "rating.submitErr": "שליחת הדירוג נכשלה. נסו שוב.",
  "rating.unavailable": "דירוגים עדיין לא זמינים בשרת.",
  "rating.orderNotFound": "ההזמנה לא נמצאה.",
  "rating.headerCount": "{n} דירוגים",
  "rating.headerAria": "ממוצע דירוג {avg} מתוך 5, {n} דירוגים",
  "rating.headerOpenList": "הצגת דירוגי לקוחות",
  "rating.listTitle": "דירוגי לקוחות",
  "rating.listLoading": "טוען דירוגים…",
  "rating.listEmpty": "אין עדיין דירוגים.",
  "rating.listClose": "סגור",
  "rating.anonymousCustomer": "לקוח",
  "rating.starsOfFive": "{n} מתוך 5",
  "rating.siteTitle": "דרגו אותנו",
  "rating.siteSubtitle": "נשמח לשמוע מה חשבתם על האוכל במסעדה",
  "rating.siteNameLabel": "שם",
  "rating.siteNamePh": "השם שלכם",
  "rating.siteNameRequired": "יש למלא שם",
  "rating.siteSendBtn": "שלח",
  "rating.siteAfterTasteBtn": "אחרי שאטעם",
  "rating.siteNeverBtn": "אל תציג שוב",
  "rating.reminderHint": "הזמנה #{order}",
  "pwa.confirmTitle": "להוסיף את Burger Hut למסך הבית?",
  "pwa.confirmBody":
    "בלחיצה אחת תקבלו אייקון בטלפון — הזמנה מהירה יותר בלי לחפש שוב את האתר.",
  "pwa.confirmInstall": "התקן",
  "pwa.confirmCancel": "לא עכשיו",
  "cat.burgers": "בורגרים",
  "cat.specials": "מיוחדים",
  "cat.crispy": "קריספי צ׳יקן",
  "cat.sides": "תוספות",
  "cat.drinks": "שתייה",

  "menu.kids-burger-120.name": "בורגר ילדים 120 גר׳",
  "menu.kids-burger-120.desc": "מנת ילדים — 120 גרם.",
  "menu.burger-160.name": "בורגר 160 גר׳",
  "menu.burger-160.desc": "בורגר קלאסי — 160 גרם.",
  "menu.burger-200.name": "בורגר 200 גר׳",
  "menu.burger-200.desc": "בורגר — 200 גרם.",
  "menu.burger-220.name": "בורגר 220 גר׳",
  "menu.burger-220.desc": "בורגר — 220 גרם.",
  "menu.smash-burger-240.name": "בורגר 240 גר׳ SMASH",
  "menu.smash-burger-240.desc":
    "סמאש — 240 גרם. קציצות במנה: 2×120 גר׳.",
  "menu.smash-burger-360.name": "בורגר 360 גר׳ SMASH",
  "menu.smash-burger-360.desc":
    "סמאש — 360 גרם. קציצות במנה: 3×120 גר׳.",
  "menu.burger-320.name": "בורגר 320 גר׳",
  "menu.burger-320.desc":
    "בורגר — 320 גרם. קציצות במנה: 2×160 גר׳.",
  "menu.burger-360.name": "בורגר 360 גר׳",
  "menu.burger-360.desc":
    "בורגר — 360 גרם. קציצות במנה: 200 + 160 גר׳.",
  "menu.burger-400.name": "בורגר 400 גר׳",
  "menu.burger-400.desc":
    "בורגר — 400 גרם. קציצות במנה: 2×200 גר׳.",
  "menu.burger-440.name": "בורגר 440 גר׳",
  "menu.burger-440.desc":
    "בורגר — 440 גרם. קציצות במנה: 2×220 גר׳.",
  "menu.burger-480.name": "בורגר 480 גר׳",
  "menu.burger-480.desc":
    "בורגר — 480 גרם. קציצות במנה: 3×160 גר׳.",
  "menu.burger-520.name": "בורגר 520 גר׳",
  "menu.burger-520.desc":
    "בורגר — 520 גרם. קציצות במנה: 200 + 160 + 160 גר׳.",
  "menu.burger-560.name": "בורגר 560 גר׳",
  "menu.burger-560.desc":
    "בורגר — 560 גרם. קציצות במנה: 200 + 200 + 160 גר׳.",
  "menu.burger-600.name": "בורגר 600 גר׳",
  "menu.burger-600.desc":
    "בורגר — 600 גרם. קציצות במנה: 3×200 גר׳ (ניתן גם 2×220 + 160 גר׳).",
  "menu.special-truffle-king.name": "Truffle King",
  "menu.special-truffle-king.desc":
    "קציצה 200 גרם, גבינת גאודה, פטריות מוקפצות, איולי כמהין, חסה ובצל סגול. טעם יוקרתי ועשיר.",
  "menu.special-truffle-king.fixedMealToppings":
    "גבינת גאודה, פטריות מוקפצות, איולי כמהין.",
  "menu.special-bbq-smoke.name": "BBQ Smoke",
  "menu.special-bbq-smoke.desc":
    "קציצה 200 גרם, אסאדו מפורק, גבינת צ׳דר, רוטב BBQ, בצל מקורמל, חמוצים וחסה. טעם מעושן ומתקתק.",
  "menu.special-bbq-smoke.fixedMealToppings":
    "אסאדו מפורק, גבינת צ׳דר, רוטב BBQ, בצל מקורמל.",
  "menu.special-fire-burger.name": "Fire Burger",
  "menu.special-fire-burger.desc":
    "קציצה 200 גרם, גבינת צ׳דר, חלפיניו, בצל מקורמל, מיונז חריף ופלפל חריף, חסה ועגבנייה. חריף ומלא טעם.",
  "menu.special-fire-burger.fixedMealToppings":
    "גבינת צ׳דר, חלפיניו, בצל מקורמל, מיונז חריף, פלפל חריף.",
  "menu.special-cheese-bomb.name": "Cheese Bomb",
  "menu.special-cheese-bomb.desc":
    "קציצה של 240 גרם ממולאת בגבינת צ׳דר, גבינת גאודה מעל, רוטב צ׳דר בצד, חסה, עגבנייה, בצל סגול וחמוצים. נמס ומפוצץ גבינה.",
  "menu.special-cheese-bomb.fixedMealToppings":
    "קציצה 240 גר׳, גבינת צ׳דר במילוי, גבינת גאודה מעל, רוטב צ׳דר בצד.",
  "menu.special-lamb-bacon-deluxe.name": "Lamb Bacon Deluxe",
  "menu.special-lamb-bacon-deluxe.desc":
    "קציצה 200 גרם, פייקון טלה קריספי, גבינת צ׳דר, בצל מקורמל, חסה, עגבנייה, מיונז ורוטב BBQ עדין. עשיר ומעושן.",
  "menu.special-lamb-bacon-deluxe.fixedMealToppings":
    "פייקון טלה קריספי, גבינת צ׳דר, בצל מקורמל, מיונז, רוטב BBQ עדין.",
  "menu.special-corned-beef-stack.name": "Corned Beef Stack",
  "menu.special-corned-beef-stack.desc":
    "קציצה 200 גרם, קורנדביף עגל, גבינת גאודה, חמוצים, בצל סגול, חסה ורוטב חרדל דבש ומיונז עדין. בסגנון דלי.",
  "menu.special-corned-beef-stack.fixedMealToppings":
    "קורנדביף עגל, גבינת גאודה, מיונז עדין.",
  "menu.special-lettuce-burger.name": "בורגר חסה",
  "menu.special-lettuce-burger.desc":
    "בורגר 160 גר׳ בהתאמה אישית — חסה במקום לחמניה, רטבים, תוספות וסלטים לבחירה. מחיר התחלה 55 ₪.",
  "menu.special-lettuce-burger.fixedMealToppings":
    "קציצת בשר 160 גר׳, עלי חסה במקום לחמניה; רטבים, תוספות וסלטים לבחירה.",
  "menu.crispy-chicken-burger.name": "קריספי צ׳יקן (עגולה/בלי עגולה)",
  "menu.crispy-chicken-burger.desc": "קריספי בעגולה או בלי עגולה.",
  "menu.crispy-chicken-burger.lineNameRound": "קריספי צ׳יקן עגולה",
  "menu.crispy-chicken-burger.lineNameNoRound": "קריספי צ׳יקן בלי עגולה",
  "menu.crispy-chicken-burger-kids.name":
    "קריספי צ׳יקן ילדים (עגולה/טורטיה קטנה)",
  "menu.crispy-chicken-burger-kids.desc": "קריספי בעגולה/טורטיה.",
  "menu.crispy-chicken-burger-kids.lineNameRoundSmall":
    "קריספי צ׳יקן ילדים עגולה קטנה",
  "menu.crispy-chicken-burger-kids.lineNameSmallTortilla":
    "קריספי צ׳יקן ילדים טורטיה קטנה",
  "menu.crispy-chicken-burger-kids.lineNameNoBread":
    "קריספי צ׳יקן ילדים בלי לחם",
  "menu.crispy-chicken-tortilla-large.name": "קריספי צ׳יקן טורטיה גדולה",
  "menu.crispy-chicken-tortilla-large.desc": "טורטיה עם עוף קריספי — מנה גדולה.",
  "menu.side-fries.name": "צ׳יפס",
  "menu.side-fries.desc": "צ׳יפס זיגזג.",
  "menu.side-sweet-potato.name": "פוטטו",
  "menu.side-sweet-potato.desc": "פוטטו.",
  "menu.side-onion-rings.name": "טבעות בצל",
  "menu.side-onion-rings.desc": "טבעות בצל פריכות.",
  "menu.side-mashed-balls.name": "כדורי פירה",
  "menu.side-mashed-balls.desc": "כדורי פירה.",
  "menu.side-mix.name": "מיקס",
  "menu.side-mix.desc": "מיקס: צ׳יפס זיגזג, פוטטו, טבעות בצל וכדורי פירה.",
  "menu.side-chips-cheddar-symphony.name": "סימפוניית צ׳יפס וצ׳דר",
  "menu.side-mozzarella-sticks.name": "אצבעות מוצרלה",
  "menu.side-mozzarella-sticks.desc": "7 אצבעות מוצרלה.",
  "menu.side-home-fries.name": "הום פרייז",
  "menu.side-home-fries.desc": "מטוגנות ברוטב צ׳ילי מתוק.",
  "menu.side-home-fries-spicy.name": "הום פרייז חריף",
  "menu.side-home-fries-spicy.desc": "מטוגנות ברוטב צ׳ילי מתוק וחריף.",
  "menu.side-chips-cheddar-symphony.desc":
    "צ׳יפס זיגזג עם רוטב גבינת צ׳דר.",
  "menu.drink-cola.name": "קוקה קולה",
  "menu.drink-cola.desc": "פחית / בקבוק קטן",
  "menu.drink-zero.name": "קולה זירו",
  "menu.drink-zero.desc": "פחית / בקבוק קטן",
  "menu.drink-sprite.name": "ספרייט",
  "menu.drink-sprite.desc": "",
  "menu.drink-fanta.name": "פאנטה",
  "menu.drink-fanta.desc": "",
  "menu.drink-tapuzina-grape.name": "תפוזינה ענבים",
  "menu.drink-tapuzina-grape.desc": "",
  "menu.drink-strawberry-banana.name": "תות בננה",
  "menu.drink-strawberry-banana.desc": "",
  "menu.drink-xl.name": "XL",
  "menu.drink-xl.desc": "משקה אנרגיה — פחית 250 מ״ל.",
  "menu.drink-xl-ten.name": "XL TEN",
  "menu.drink-xl-ten.desc":
    "משקה אנרגיה מוגז ללא סוכר — פחית 250 מ״ל (10 קלוריות).",
  "menu.drink-blue-mojito.name": "BLUE מוחיטו",
  "menu.drink-blue-mojito.desc": "משקה אנרגיה BLUE בטעם מוחיטו.",
  "menu.drink-soda.name": "סודה",
  "menu.drink-soda.desc": "Schweppes — מי סודה מוגז.",
  "menu.drink-water-small.name": "מיי עדן",
  "menu.drink-water-small.desc": "",
  "menu.drink-fresh-strawberry.name": "fresh תות",
  "menu.drink-fresh-strawberry.desc":
    "Fresher — טעם תות, מי מינרל טבעיים וויטמינים.",
  "menu.drink-cola-15l.name": "קוקה קולה 1.5 ליטר",
  "menu.drink-cola-15l.desc": "בקבוק משפחתי",
  "menu.drink-zero-15l.name": "קוקה קולה זירו 1.5 ליטר",
  "menu.drink-zero-15l.desc": "בקבוק משפחתי",

  "topping.cheddar": "גבינת צ׳דר",
  "topping.gouda": "גאודה",
  "topping.fried_egg": "ביצת עין",
  "topping.jalapeno": "חלפיניו",
  "topping.caramel_mushrooms": "פטריות מקורמל",
  "topping.caramel_onion": "בצל מקורמל",
  "topping.avocado": "אבוקדו",
  "topping.lamb_bacon": "בייקון טלה",
  "topping.veal_corned": "קורנדביף עגל",
  "topping.pulled_asado": "אסאדו מפורק",

  "salad.salad_none": "ללא סלטים",
  "salad.salad_lettuce": "חסה",
  "salad.salad_tomato": "עגבניה",
  "salad.salad_pickles": "חמוצים",
  "salad.salad_onion": "בצל",
  "salad.salad_coleslaw": "סלט קולסלאו",

  "sauce.sauce_house_regular": "רוטב הבית רגיל",
  "sauce.sauce_house_spicy": "רוטב הבית חריף",
  "sauce.sauce_bbq": "ברביקיו",
  "sauce.sauce_sweet_chili": "צ׳ילי מתוק",
  "sauce.sauce_hot_chili": "צ׳ילי חריף",
  "sauce.sauce_garlic": "שום",
  "sauce.sauce_cheddar": "רוטב צ׳דר",

  "ui.freeSalads": "סלטים",
  "ui.adultCrispyNoRound": "בלי עגולה",
  "ui.kidsCrispyBread.title": "עגולה קטנה / טורטיה קטנה / בלי לחם",
  "ui.kidsCrispyBread.round": "עגולה קטנה",
  "ui.kidsCrispyBread.small_tortilla": "טורטיה קטנה",
  "ui.kidsCrispyBread.none": "בלי לחם",
  "ui.saucePricingHint":
    "בחר רוטב אחד ללא עלות. מרוטב שני ₪4 לכל רוטב. רוטב צ׳דר ₪8.",
  "ui.saucePlus": "+₪",
  "ui.saladOutOfStock": "(אזל מהמלאי)",
  "ui.menuPattyRemainingFew": "נותר {n} מנות",
  "ui.sauceAddOne": "הוספת מנת רוטב",
  "ui.sauceRemoveOne": "הפחתת מנת רוטב",
  "ui.doubleWord": "דבל",
  "ui.bunSauceOnBunTitle": "רוטב על הלחמניה?",
  "ui.bunSauceYes": "כן",
  "ui.bunSauceNo": "לא",
  "ui.donenessTitle": "מידת העשייה",
  "ui.doneness.rare": "Rare",
  "ui.doneness.medium": "Medium",
  "ui.doneness.medium_well": "Medium well",
  "ui.doneness.well_done": "Well done",
  "ui.burgerToppings": "תוספות לבורגר",
  "ui.crispyToppings": "תוספות לקריספי",
  "ui.extraSauces": "תוספות רטבים בצד",
  "ui.addToCart": "הוספה לעגלה",
  "ui.updateMealInCart": "עדכן בסל",
  "ui.pattyInsufficientForMeal":
    "מסוג המנה הזאת נותרו לפי המלאי: {count}.\nהורידו את הכמות או בחרו מנה אחרת.",
  "ui.pattyInsufficientForMealFallback":
    "לא ניתן להוסיף את הכמות לפי מלאי הקציצות והעגלה. הורידו כמות או בחרו מנה אחרת.",
  "ui.pattyInsufficientCapOne":
    "מסוג זה יש רק מנה אחת לפי המלאי. הורידו את הכמות ל־1 או בחרו מנה אחרת.",
  "ui.pattyInsufficientCapMany":
    "מסוג זה ניתן להזמין לכל היותר {max} מנות לפי המלאי. הורידו את הכמות ל־{max} או בחרו מנה אחרת.",
  "ui.pattyInsufficientTypeExhausted":
    "מסוג זה אין כרגע מלאי לפי הקציצות (בהתחשב בשאר העגלה). בחרו מנה אחרת או הורידו כמות במנה אחרת בעגלה.",
  "ui.pattyStockCheckFailed":
    "לא ניתן לבדוק את מלאי הקציצות כרגע. נסו שוב בעוד רגע.",
  "ui.pattyCartHavePattyZero": "אין במלאי קציצות {g} גר'",
  "ui.pattyCartHavePattyOne": "קציצת {g} גר' אחת",
  "ui.pattyCartHavePattyMany": "{have} קציצות {g} גר'",
  "ui.pattyCartBumpShort":
    "{stock}. ניסית להוסיף עוד מנה מסוג «{name}» — לא ניתן, בחר מנה אחרת.",
  "ui.pattyCartUnknownItem": "המנה",
  "ui.added": "נוסף",
  "ui.mealValidateTitle": "חסרות בחירות",
  "ui.mealValidateSalads": "לא בחרת מסלטים.",
  "ui.mealValidateSauces":
    "לא בחרת מתוספות רטבים בצד מגיע לך רוטב אחד חינם.",
  "ui.mealValidateFries": "לא בחרת מטוגנים למנה. (חובה לבחור)",
  "ui.mealValidateAddAnyway": "אני יודע הוסף לסל",
  "ui.mealValidateGoBack": "חזור לבחירה",
  "ui.outOfStock": "היום אין יותר ממנה זאת",
  "ui.soldOutShort": "אזל",
  "ui.openCustomize": "בחירת סלטים, תוספות, רטבים, מטוגנים",
  "ui.editComponents": "ערוך את המרכיבים",
  "ui.fromPrice": "החל מ־",
  "ui.wizardClose": "סגור",
  "ui.wizardStep": "שלב",
  "ui.wizardNext": "הבא",
  "ui.wizardBack": "חזרה",
  "ui.wizardCancel": "ביטול",
  "ui.wizardStepAdd": "כמות והוספה לעגלה",
  "ui.wizardServingTitle": "אפשרויות הגשה",
  "ui.wizardPriceLine": "מחיר ליחידה (לפי הבחירה):",
  "ui.wizardAllOnOneScreen":
    "כל האפשרויות למטה — גללו, בחרו והוסיפו לעגלה בתחתית המסך.",
  "ui.wizardSpecialFullCustomizeHint":
    "מרכיבי המנה לפי המתכון מוצגים למטה. אפשר גם להוסיף תוספות לבורגר, לערוך סלטים, עשייה, רטבים, מטוגנים ושתייה — גללו והוסיפו לעגלה בתחתית.",
  "ui.wizardSpecialSaladsOnly":
    "תוספות המנה (גבינה, מה שמגיע על המנה וכו׳) קבועות לפי המתכון. במנה קציצה אחת — בוחרים משקל 200 או 220 גר׳ למטה. ניתן לערוך גם סלטים, מידת עשייה, רטבים בצד ושתייה — גללו והוסיפו לעגלה למטה.",
  "ui.wizardSpecialSaladsOnlyCheeseBomb":
    "תוספות המנה קבועות לפי המתכון. קציצה 240 גר׳. ניתן לערוך סלטים, מידת עשייה, רטבים בצד ושתייה — גללו והוסיפו לעגלה למטה.",
  "ui.specialPattyTitle": "משקל הקציצה",
  "ui.specialPatty200": "200 גר׳",
  "ui.specialPatty220": "220 גר׳ (+₪5)",
  "ui.specialPatty200Depleted": "200 גר׳ — אזל",
  "ui.specialPatty220Depleted": "220 גר׳ (+₪5) — אזל",
  "ui.specialFixedToppingsTitle": "תוספות במנה",
  "ui.specialFixedToppingsHint": "לא ניתן לשנות",
  "ui.mealFriesForMealLabel": "בחר מטוגנים למנה:",
  "ui.mealFriesSelectPlaceholder": "בחר",
  "ui.mealFriesMultiHint": "אפשר לבחור כמה סוגים יחד.",
  "ui.mealFriesValidate": "בחרו לפחות אפשרות אחת למטוגנים למנה לפני ההוספה לעגלה.",
  "ui.mealFries.none": "ללא מטוגנים",
  "ui.mealFries.regular": "צ'יפס",
  "ui.mealFries.mix": "מיקס הבית",
  "ui.mealFries.onion-rings": "טבעות בצל",
  "ui.mealFries.potato-balls": "כדורי פירה",
  "ui.mealFries.potato": "פוטטו",
  "ui.mealFries.chips-cheddar-symphony": "סימפוניית צ'יפס וצ'דר",
  "ui.mealFries.mozzarella-sticks": "אצבעות מוצרלה",
  "ui.mealFries.home-fries": "הום פרייז",
  "ui.mealFries.home-fries-spicy": "הום פרייז חריף",
  "ui.addDrinkQuestion": "להוסיף שתיה?",
  "ui.addDrinkSelectPlaceholder": "בחרו משקה מהרשימה",
  "ui.sellerNotes": "הערות למוכר",
  "ui.sellerNotesPh": "למשל: בלי רטבים, אריזה נפרדת…",

  "cart.label": "עגלה",
  "cart.items": "פריטים",
  "cart.unitsOne": "פריט אחד",
  "cart.unitsMany": "{count} פריטים",
  "cart.checkout": "לתשלום",
  "cart.showCart": "הצג עגלה",
  "cart.hideCart": "הסתר עגלה",
  "cart.panelTitle": "העגלה שלך",
  "cart.closePanel": "סגור",
  "cart.editMeal": "עריכה",
  "cart.menuStillVisibleHint":
    "ניתן לגלול למעלה — התפריט והקטגוריות נשארים זמינים.",

  "checkout.title": "סיכום והמשך לתשלום",
  "checkout.backToMenu": "חזרה לתפריט",
  "checkout.subtitle": "בדקו את הפרטים ובחרו אמצעי תשלום.",
  "checkout.emptyCart": "העגלה ריקה. הוסיפו מוצרים מהתפריט.",
  "checkout.summary": "סיכום עגלה",
  "checkout.size": "גודל",
  "checkout.variant": "הגשה",
  "checkout.saladsPrefix": "סלטים",
  "checkout.noSalads": "ללא סלטים",
  "checkout.specialMealComponentsPrefix": "מרכיבים",
  "checkout.bunSauceOnBunPrefix": "רוטב על הלחמניה",
  "checkout.donenessPrefix": "מידת העשייה",
  "checkout.toppingsPrefix": "תוספות לבורגר",
  "checkout.extrasPrefix": "תוספות רטבים בצד",
  "checkout.mealFriesPrefix": "מטוגנים למנה",
  "checkout.mealFriesCartSummaryTitle": "סיכום מטוגנים למנות",
  "checkout.unitPrice": "מחיר ליחידה",
  "checkout.remove": "מחק",
  "checkout.grandTotal": "סה״כ",
  "checkout.customer": "פרטי לקוח",
  "checkout.firstName": "שם פרטי",
  "checkout.lastName": "שם משפחה",
  "checkout.firstNamePh": "הקלידו שם פרטי",
  "checkout.lastNamePh": "הקלידו שם משפחה",
  "checkout.phone": "טלפון",
  "checkout.saveContactQuestion": "לשמור את פרטי הלקוח לפעם הבאה?",
  "checkout.saveContactYes": "כן",
  "checkout.saveContactNo": "לא",
  "checkout.emailOptional": "אימייל (לחשבונית)",
  "checkout.emailPlaceholder": "לדוגמה: name@example.com",
  "checkout.orderType": "סוג הזמנה",
  "checkout.delivery": "משלוח",
  "checkout.pickup": "איסוף מהמסעדה",
  "checkout.address": "כתובת מלאה",
  "checkout.addressPh": "עיר, רחוב, דירה, הערות למשלוח",
  "checkout.payment": "תשלום",
  "checkout.restaurantClosedNow": "המסעדה סגורה כעת.",
  "checkout.restaurantClosedHoursHint": "ראה זמני עבודה בדף הראשי",
  "checkout.submit": "המשך",
  "checkout.submitting": "מעבדים…",
  "checkout.cardHypNotConfigured":
    "תשלום אשראי אינו מוגדר בשרת. פנו לחנות או בחרו מזומן/ביט. (בעלי האתר: יש להשלים משתני Hyp בקובץ .env.local או ב-Vercel.)",
  "checkout.cardHypMissingLine": "חסרים בשרת",
  "checkout.cardMissingSiteUrl":
    "חסר כתובת האתר לצורך חזרה מתשלום. הגדירו NEXT_PUBLIC_SITE_URL (או SITE_URL) ב-.env.local.",
  "checkout.cardPaymentInitFailed":
    "לא ניתן לפתוח דף תשלום. נסו שוב או בחרו אמצעי תשלום אחר.",
  "checkout.pickupBranch": "איסוף מהמסעדה",
  "checkout.deliveryWhere": "אזור משלוח",
  "checkout.zoneYarka": "ירכא",
  "checkout.zoneOutside": "מחוץ לירכא",
  "checkout.addressYarkaHint":
    "כתובת מדויקת בירכא או למסור לשליח בטלפון",
  "checkout.addressOutsideHint":
    "כתובת למשלוח - עיר, רחוב..",
  "checkout.calcDelivery": "חישוב דמי משלוח (מסלול כביש)",
  "checkout.geocoding": "מאתרים כתובת…",
  "checkout.geocodeFail": "לא נמצאה כתובת. נסו לפרט עיר ורחוב.",
  "checkout.deliveryFeeLine": "דמי משלוח",
  "checkout.foodSubtotal": "סה״כ מנות",
  "checkout.discountLine": "הנחה",
  "checkout.couponTitle": "קופון",
  "checkout.couponPh": "יש לכם קופון הנחה? הקלידו את הקוד כאן…",
  "checkout.couponApply": "בדיקה",
  "checkout.couponChecking": "בודק…",
  "checkout.couponApplied": "הקופון תקין והופעל.",
  "checkout.couponInvalid": "קוד קופון לא תקין.",
  "checkout.couponExpired": "קוד קופון פג תוקף.",
  "checkout.couponUsed": "קוד קופון כבר נוצל.",
  "checkout.couponInvalidSubmit":
    "קוד קופון לא תקין. הסירו/עדכנו קופון ונסו שוב.",
  "checkout.couponExpiredSubmit":
    "קוד קופון פג תוקף. הסירו/עדכנו קופון ונסו שוב.",
  "checkout.couponUsedSubmit":
    "קוד קופון כבר נוצל. הסירו/עדכנו קופון ונסו שוב.",
  "checkout.couponLine": "קופון",
  "checkout.deliveryPayHow": "תשלום עבור המשלוח",
  "checkout.payRestaurantAll": "לשלם למסעדה (מזון + משלוח)",
  "checkout.payCourierDeliveryFoodPrefix": "לשלם למסעדה עלות המזון (",
  "checkout.payCourierDeliveryFoodSuffix":
    ") ושלם לשליח דמי משלוח במזומן",
  "checkout.deliveryPayCashTitle": "תשלום במזומן (משלוח)",
  "checkout.cashCourierFullHint":
    "במזומן יש לשלם לשליח את סכום המזון ודמי המשלוח יחד — במזומן, בביקור המשלוח.",
  "checkout.cashCourierTotalLabel": "סה״כ לתשלום לשליח",
  "checkout.deliveryPayOnlineTitle": "אשראי / ביט — חלוקת תשלום המשלוח",
  "checkout.deliveryPayOnlineHint":
    "בחרו לפני המשך לתשלום: האם לחייב אונליין את כל הסכום (מזון + משלוח) או רק את המזון, ודמי המשלוח במזומן לשליח.",
  "checkout.onlinePayFoodOnlyLabel": "סכום לחיוב באשראי או ביט (מזון בלבד):",
  "checkout.onlinePayDeliveryToCourier":
    "דמי המשלוח יש לשלם לשליח במזומן.",
  "checkout.onlinePayFullLabel": "סכום לחיוב באשראי או ביט (מזון + משלוח):",
  "checkout.payCourierCashFull": "מזומן לשליח — מזון + משלוח",
  "checkout.distanceKm": "מרחק מהמסעדה",
  "checkout.drivingDistanceKm": "מרחק נסיעה משוער (כביש)",
  "checkout.airFallbackNote":
    "לא נמצא מסלול בכבישים — חויב לפי מרחק אווירי. נסו לדייק בכתובת.",
  "checkout.addressPickHint": "בחרו כתובת מהרשימה לתוצאה מדויקת יותר:",
  "checkout.mapOpenField": "לחצו לפתיחת מפה ובחירת נקודת משלוח",
  "checkout.mapChange": "שינוי מיקום במפה",
  "checkout.mapTitle": "בחירת כתובת במפה",
  "checkout.pickLocation": "בחר מיקום",
  "checkout.mapTapHint":
    "הזיזו את המפה או לחצו «בחר המיקום שלי». הגבולות הצבעוניים הם אזורי המשלוח: ירכא ₪25, גוליס ₪30, אבו סנאן ₪35, כפר יסיף ₪40, גת ₪35, יאנוח ₪40.",
  "checkout.deliveryVillageLine": "יישוב",
  "checkout.outOfVillageTitle": "מחוץ לאזור המשלוח המתומחר",
  "checkout.outOfVillageBody": "התקשרו למסעדה לברר עלות משלוח",
  "checkout.outOfVillageCall": "התקשרו למסעדה",
  "checkout.outOfVillageClose": "סגירה",
  "checkout.outOfVillagePayCash":
    "אני מודע לעלות המשלוח ואשלם במזומן לשליח",
  "checkout.deliveryFeeAgreedValue": "תואמה עם המסעדה",
  "checkout.agreedDeliveryPayHint":
    "דמי המשלוח ישולמו במזומן לשליח לפי מה שתואם עם המסעדה. סכום המזון לפי אמצעי התשלום שנבחר.",
  "checkout.mapUseMyLocation": "בחר המיקום שלי",
  "checkout.mapLocating": "מאתרים… אם כרום שואל למעלה — לחצו אפשר.",
  "checkout.mapLocateDenied":
    "כרום חסם מיקום לאתר {host} (ה-GPS בטלפון יכול להיות דלוק). לחצו על המנעול ליד הכתובת → הגדרות אתר → מיקום → אפשר, ואז לחצו שוב.",
  "checkout.mapLocateUnavailable":
    "לא הצלחנו לקבל את המיקום. בדקו ששירות המיקום מופעל ונסו שוב.",
  "checkout.mapLocateTimeout": "פג הזמן לאיתור המיקום. נסו שוב.",
  "checkout.mapLocateUnsupported": "הדפדפן לא תומך בשירות מיקום.",
  "checkout.mapStreet": "מפה",
  "checkout.mapSatellite": "לוויין",
  "checkout.mapConfirm": "אישור והחלת מיקום",
  "checkout.mapCancel": "ביטול",
  "checkout.mapApplying": "מחשבים משלוח…",
  "checkout.mapAddressFallback":
    "נקודה מהמפה — נא לפרט רחוב, מספר בית והערות בשדה ההערות למטה",
  "checkout.mapSelectHint": "בחרו נקודת משלוח במפה כדי לחשב דמי משלוח.",
  "checkout.outsideNotesOptional": "הערות למשלוח (אופציונלי)",
  "checkout.outsideNotesPh": "קומה, כניסה, הוראות לשליח…",
  "checkout.mapErrBounds": "הנקודה מחוץ לאזור השירות. בחרו מיקום בישראל.",
  "checkout.mapErrFee": "לא ניתן לחשב דמי משלוח לנקודה זו.",
  "checkout.mapErrGeneric": "לא הצלחנו לחשב את המשלוח. נסו נקודה אחרת.",
  "checkout.mapErrNetwork": "שגיאת רשת. בדקו חיבור ונסו שוב.",
  "checkout.mapErrBadCoords":
    "נקודה לא תקינה — לחצו שוב על המפה או זזו מעט והבחרו מחדש.",
  "checkout.mapErrServer": "שגיאת שרת בחישוב המשלוח. נסו שוב בעוד רגע.",
  "checkout.mapErrBadQuery": "בעיה בבקשה — רעננו את הדף ונסו שוב.",

  "err.firstName": "שם פרטי חובה",
  "err.lastName": "שם משפחה חובה",
  "err.phone": "טלפון חובה",
  "err.address": "כתובת נדרשת למשלוח (מחוץ לירכא)",
  "err.mapRequired": "יש לבחור נקודת משלוח על המפה",
  "err.deliveryZone": "בחרו אזור משלוח (ירכא / מחוץ לירכא)",
  "err.deliveryGeocode": "יש לאשר מיקום במפה (חישוב משלוח לא הושלם)",
  "err.deliveryPayTo":
    "בחרו באשראי/ביט: תשלום מלא (מזון+משלוח) או מזון בלבד ודמי משלוח לשליח",
  "err.cart": "העגלה ריקה",
  "err.unavailable":
    "יש בעגלה מנה שאזלה מהמלאי — הסירו אותה או הורידו כמות והמשיכו.",
  "err.insufficientPatties":
    "אין מספיק קציצות במלאי להשלמת ההזמנה — עדכנו את העגלה או נסו שוב מאוחר יותר.",
  "err.insufficientPattiesIntro": "לא ניתן להשלים את ההזמנה — חסר במלאי:",
  "err.insufficientPattiesGram": "קציצות {g} גר׳ (נדרש {need}, זמין {have})",
  "err.insufficientPattiesGramSep": " · ",
  "err.insufficientPattiesMealsIntro": "מנות בעגלה הרלוונטיות:",
  "err.insufficientPattiesMealItem": "×{qty} {name}",
  "err.insufficientPattiesMealSep": ", ",
  "err.insufficientPattiesAction":
    "עדכנו את העגלה (הסירו מנה או הורידו כמות) או נסו שוב מאוחר יותר.",
  "err.orderingClosed":
    "ניתן להזמין משעה 10:00 עד שעת הסגירה באותו יום. תחילת עבודה על ההזמנות בשעה 16:00",

  "payment.cash": "מזומן",
  "payment.card": "אשראי",
  "payment.bit": "ביט",
  "payment.backToCheckout": "חזרה לבחירת אמצעי תשלום",

  "card.title": "תשלום באשראי",
  "card.subtitle":
    "כאן אפשר לחזור לעגלה ולבחור מזומן או ביט, או להמשיך לאתר הסולק.",
  "card.amountLabel": "סכום לחיוב",
  "card.orderRef": "אסמכתא להזמנה",
  "card.continueExternal": "המשך לתשלום בכרטיס אשראי",
  "card.openingPayment": "פותחים דף תשלום…",
  "card.paymentStartFailed": "לא ניתן להתחבר לסולק. נסו שוב או בחרו אמצעי אחר.",
  "card.errorTechnical": "פרט טכני",
  "card.errorHypCode": "קוד תשובה מהסולק",
  "card.hintDnsNotFound":
    "בעיית רשת/DNS לשרת Hyp Pay (pay.hyp.co.il). בדקו חיבור לאינטרנט, או את HYP_PAY_BASE אם שיניתם אותו.",
  "card.errNoSession":
    "אין נתוני תשלום. חזרו לעגלה והשלימו את ההזמנה מחדש.",

  "success.paymentCompleted": "התשלום בוצע בהצלחה 🎉",
  "success.paymentCompletedSub": "תודה! ההזמנה נרשמה.",
  "success.titleOnline": "מעבר לדף התשלום",
  "success.titleOk": "ההזמנה התקבלה!",
  "success.descOnline": "בסביבת ייצור תועברו לספק תשלום (PayMe / Tranzila).",
  "success.descBit": "תודה! ההזמנה נרשמה.",
  "success.descCash":
    "תודה! ההזמנה נרשמה.",
  "success.descCard":
    "תודה! התשלום התקבל וההזמנה נרשמה במערכת.",
  "success.invoiceSentEmailAndSms":
    "החשבונית נשלחה לכתובת האימייל ולמספר הטלפון שהוזנו בהזמנה (אם הופעלו חשבוניות ו־SMS במסוף).",
  "success.invoiceSentEmailOnly":
    "החשבונית נשלחה לכתובת האימייל שהוזנה בהזמנה (אם הופעל מודול חשבוניות במסוף).",
  "success.invoiceSentSmsOnly":
    "החשבונית נשלחה ב־SMS למספר הטלפון שהוזן בהזמנה (אם הופעלו חשבוניות וחבילת SMS במסוף).",
  "success.invoiceCheckoutHintNoChannel":
    "לא נשלחה חשבונית מהמסוף: בשדה «אימייל (לא חובה)» בצ'קאאוט צריך להופיע אימייל תקין כדי שהאתר יבקש מהמסוף חשבונית במייל, או מספר נייד ישראלי תקין ל־SMS. בלי אחד מאלה המסוף לא מקבל בקשה לשליחת חשבונית.",
  "success.invoiceHypNoReceipt":
    "האתר ביקש חשבונית לפי הפרטים שהזנתם, אך בכתובת החזרה מהמסוף לא מופיע מספר חשבונית (Hesh). בדקו במסוף Hyp שהופעל מודול חשבוניות ודוא״ל, בתיקיית דואר נכנס ובספאם. סכום קטן (למשל 1 ₪) לא אמור למנוע זאת בקוד האתר.",
  "success.waSendOrder": "שלח לווטסאפ",
  "success.waCashHeadline": "הזמנתך נרשמה במערכת",
  "success.waCashSubline": "שלח אותה כדי לתאם זמן הכנה וקבלה",
  "success.waCardHeadline": "מעולה! יש לנו הזמנה",
  "success.waCardNowLine": "עכשיו:",
  "success.waOrderRecordedWithId": "הזמנתך #{order} מוכנה לשליחה",
  "success.waOrderRecordedNoId": "הזמנתך מוכנה לשליחה",
  "success.loadingProgressAria": "טוען, נא להמתין",
  "success.couponTitle": "🎁 קיבלת קופון!",
  "success.couponValue": "💰 ₪{value} הנחה",
  "success.couponCode": "🔢 קוד: {code}",
  "success.couponExpiry": "⏳ בתוקף עד: {date}",
  "success.couponRedeemSite": "למימוש כנסו ל־burgerhut.co.il",
  "success.couponCopy": "העתקת קוד",
  "success.couponCopied": "הקוד הועתק",
  "success.couponScreenshotHint":
    "צלם את המסך כדי לשמור את הקופון לשימוש בפעם הבאה!",
  "success.back": "חזרה לתפריט",

  "cancel.title": "התשלום בוטל",
  "cancel.desc":
    "לא בוצע חיוב. ניתן לחזור לעגלה ולהשלים הזמנה באמצעי תשלום אחר.",
  "cancel.backMenu": "חזרה לתפריט",
  "cancel.backCheckout": "חזרה לתשלום",

  "paymentError.title": "התשלום לא הושלם",
  "paymentError.desc":
    "אירעה שגיאה בתשלום. לא נגרע כסף או שהעסקה נדחתה. נסו שוב או בחרו אמצעי אחר.",
  "paymentError.backMenu": "חזרה לתפריט",
  "paymentError.backCheckout": "חזרה לתשלום",

  "bit.title": "תשלום בביט",
  "bit.subtitle": "שלמו באפליקציית ביט בטלפון (האפליקציה חייבת להיות מותקנת).",
  "bit.payIntro":
    "זה המספר שלנו לקבלת ביט: 0504847599\nשמרו אותו בטלפון שלכם בורגר האט\nוהעבירו את הסכום — עלות ההזמנה: ₪{amount}",
  "bit.payForOrderLine": "תשלום עבור הזמנה מספר: {orderNumber}",
  "bit.copyNumber": "העתק מספר",
  "bit.copyAmount": "העתק סכום",
  "bit.openApp": "פתיחת אפליקציית ביט",
  "bit.paidCheckbox": "שילמתי בביט",
  "bit.errNotifyAdmin": "לא ניתן לשלוח את ההזמנה לניהול. נסו שוב.",
  "bit.waOpening": "פותח…",
  "bit.errNoSession": "אין נתוני הזמנה. חזרו לעגלה והשלימו שוב.",
  "bit.errBadJson": "נתוני הזמנה לא תקינים.",
  "bit.errEmptyCart": "העגלה שמורה ריקה.",
  "bit.tryOpen": "מנסים לפתוח את ביט…",
  "bit.openFallback": "אם ביט לא נפתח — שלמו ידנית ואז שלחו את ההזמנה בווטסאפ.",
  "bit.copied": "הועתק",
  "bit.copyFail": "לא ניתן היה להעתיק אוטומטית — העתיקו ידנית.",

  "float.wa": "יצירת קשר בווטסאפ",

  "wa.brand": "*Burger Hut בורגר האט*",
  "wa.newOrder": "הזמנה חדשה:",
  "wa.orderNumber": "מספר הזמנה",
  "wa.name": "שם",
  "wa.phone": "טלפון",
  "wa.address": "כתובת",
  "wa.orderType": "סוג הזמנה",
  "wa.pickup": "איסוף מהמסעדה",
  "wa.delivery": "משלוח",
  "wa.payment": "אמצעי תשלום",
  "wa.details": "פרטי ההזמנה",
  "wa.nonMealItemsHeader": "תוספות, צ'יפס ושתייה:",
  "wa.toppings": "תוספות לבורגר",
  "wa.toppingsCrispy": "תוספות לקריספי",
  "wa.salads": "סלטים",
  "wa.bunSauceOnBun": "רוטב על הלחמניה",
  "wa.doneness": "מידת העשייה",
  "wa.specialPatty": "משקל קציצה",
  "wa.cheeseBombPattyLine": "240 גר׳",
  "wa.gramsUnit": " גר׳",
  "wa.sauces": "תוספות רטבים בצד",
  "wa.linePrice": "מחיר",
  "wa.drink": "שתיה",
  "wa.mealFries": "מטוגנים למנה",
  "wa.sellerNotes": "הערות למוכר",
  "wa.total": "סה״כ",
  "wa.foodSubtotal": "סה״כ מנות",
  "wa.discount": "הנחת מבצע",
  "wa.coupon": "קופון",
  "wa.couponDiscountLabel": "הנחת קופון",
  "wa.couponCodeLabel": "קוד קופון",
  "wa.nisCurrencySuffix": "ש״ח",
  "wa.totalWithDelivery": "סה״כ כולל משלוח",
  "wa.deliveryArea": "אזור משלוח",
  "wa.zoneYarka": "ירכא",
  "wa.zoneOutside": "מחוץ לירכא",
  "wa.airDistance": "מרחק אווירי (משוער)",
  "wa.drivingDistance": "מרחק נסיעה משוער (כביש)",
  "wa.deliveryDistanceApprox": "מרחק משוער",
  "wa.deliveryVillage": "יישוב למשלוח",
  "wa.deliveryFeeAgreed": "תואמה עם המסעדה",
  "wa.deliveryFee": "דמי משלוח",
  "wa.payRestaurantInclDelivery": "תשלום למסעדה כולל דמי משלוח",
  "wa.payCourierAgreed":
    "דמי משלוח במזומן לשליח — תואמה עם המסעדה",
  "wa.payCourierCashFull":
    "מזומן לשליח: סכום מלא (מזון + דמי משלוח) — לשלם לשליח בביקור",

  "admin.title": "ניהול מסעדה — Burger Hut",
  "admin.backHome": "חזרה לדף הבית",
  "admin.hint": "דף פנימי למנהל בלבד. אל תשתף קישור או סיסמה עם לקוחות.",
  "admin.employeeBadge": "מצב עובד",
  "admin.employeeHint":
    "מצב עובד — מלאי, שעות פעילות, הזמנות היום ומכירות אתמול (ללא ארכיון מלא).",
  "admin.employeeOrdersTitle": "הזמנות ומכירות",
  "admin.employeeTodayBtn": "היום",
  "admin.employeeYesterdayBtn": "אתמול",
  "admin.employeeAutoRefreshHint":
    "ההזמנות להיום ולאתמול מתעדכנות אוטומטית כל כ־15 שניות כשהדף פתוח.",
  "admin.secretStorageHint":
    "אחרי «טען» המערכת שומרת את קוד הניהול רק בדפדפן/מכשיר שלך (לא בשרת). מבקר אחר באתר לא רואה ולא מקבל את הקוד — אלא אם הוא משתמש באותו מכשיר שבו כבר נכנסת. במכשיר משותף לחצו «התנתק» כשסיימתם.",
  "admin.logoutBtn": "התנתק ומחק קוד מהמכשיר",
  "admin.closeStore": "סגור חנות",
  "admin.openStore": "פתח חנות",
  "admin.storeCloseOutsideHours":
    "ניתן לסגור או לפתוח את החנות רק בשעות העבודה.",
  "admin.storeCloseErr": "לא ניתן לעדכן את מצב החנות.",
  "admin.logoutConfirm":
    "למחוק את קוד הניהול מהדפדפן ולסגור את הפאנל? (תצטרכו להזין שוב בפעם הבאה)",
  "admin.newOrderAutoRefreshHint":
    "ההזמנות והמכירות לפי יום מתעדכנות אוטומטית כל כ־15 שניות כשהדף פתוח.",
  "admin.newOrderNotifyHint":
    "לחיצה תפעיל התראות במסך, צליל ורטט, וגם התראות Push כשהאפליקציה סגורה — אם הוגדרו בשרת מפתחות VAPID ו־Redis (ראו .env.example).",
  "admin.newOrderNotifyEnable": "אפשר התראות",
  "admin.newOrderNotifyDenied":
    "התראות חסומות בהגדרות הדפדפן. אפשר לאפשר בהגדרות האתר.",
  "admin.newOrderNotifyDeniedHelp":
    "Chrome / Edge (מחשב): סמל המנעול או «ⓘ» ליד הכתובת → «הגדרות לאתר הזה» / Site settings → התראות → «אפשר». לאחר מכן רעננו את הדף. אנדרואיד (כרום): ⋮ → הגדרות → הגדרות אתר → התראות; או הגדרות המכשיר → אפליקציות → כרום → התראות. אייפון (אתר או PWA מהמסך הראשי): הגדרות → התראות → בחרו את האפליקציה או את Safari/דפדפן הרלוונטי → אפשרו; ל־PWA מותקן לפעמים תחת שם האתר או «התראות אפליקציות» (נדרש iOS 16.4+ ל־Web Push).",
  "admin.newOrderNotifyTitle": "הזמנה חדשה — Burger Hut",
  "admin.newOrderNotifyBodyOne": "התקבלה הזמנה מס׳ #{n}.",
  "admin.newOrderNotifyBodyNoNum": "התקבלה הזמנה חדשה.",
  "admin.newOrderNotifyCouponDiscount": " הנחת קופון: {amount} ש״ח.",
  "admin.newOrderNotifyCouponCode": " קוד קופון: {code}.",
  "admin.newOrderNotifyBodyMany": "התקבלו {c} הזמנות חדשות.",
  "admin.pushSubscribeOk":
    "התראות Push הופעלו — תקבלו התראה גם כשהאפליקציה סגורה (אם המכשיר והדפדפן מאפשרים).",
  "admin.pushSubscribeSkip":
    "התראות במסך פעילות. Push ברקע לא הוגדר בשרת (VAPID) או אין Redis — ראו הנחיות ב־.env.",
  "admin.pushSubscribeErr": "לא ניתן היה לרשום Push. נסו שוב או בדקו חיבור.",
  "admin.pushSubscribeRedis":
    "Redis/KV מחובר אבל שמירת המנוי נכשלה. ודאו ש־Redis זמין.",
  "admin.pushSubscribeClientId":
    "לא ניתן לשמור מזהה מכשיר (למשל דפדפן חוסם אחסון מקומי). אפשרו לאתר localStorage או נסו דפדפן/מצב רגיל.",
  "admin.newOrderNotifyGrantedHint":
    "התראות במסך מופעלות. למכשיר נייד מומלץ גם לרשום Push כדי לקבל התראה כשהאפליקציה סגורה.",
  "admin.pushRegisterBtn": "רשום Push ברקע",
  "admin.pushReregisterBtn": "עדכן רישום Push (מחדש)",
  "admin.pushLocalActive":
    "במכשיר הזה כבר רשום Push — לא חובה ללחוץ שוב בכל פתיחה של הניהול. אם שיניתם מפתחות בשרת או שההתראות לא מגיעות, לחצו «עדכן רישום».",
  "admin.pushCheckingLocal": "בודקים אם המכשיר רשום ל-Push…",
  "admin.pushPerDeviceHint":
    "חשוב: כל מכשיר בנפרד — התראה בטלפון רק אחרי «רשום Push» בטלפון. המספר למעלה = כמה כתובות Push שונות יקבלו התראה (לא «שדות» גולמיים בבסיס). רישום מחדש מנקה כפילויות לאותו מכשיר. אם עדיין יש יותר מדי: מחקו את כל המנויים ואז רשמו רק במכשירים הנדרשים.",
  "admin.pushStatusLine":
    "סטטוס שרת להתראות ברקע: VAPID ‏{vapid} · Redis ‏{redis} · יעדי Push ייחודיים (מכשירים): ‏{count}.",
  "admin.pushStatusOn": "מוגדר",
  "admin.pushStatusOff": "לא מוגדר",
  "admin.pushClearAllBtn": "מחק את כל מנויי Push מהשרת",
  "admin.pushClearAllConfirm":
    "למחוק את כל מנויי ההתראות (Push) השמורים בשרת? אחרי מחיקה צריך לרשום מחדש «רשום Push ברקע» בכל מכשיר שצריך התראות.",
  "admin.pushClearAllOk": "כל המנויים נמחקו מהשרת. רשמו מחדש Push במכשירים הנדרשים.",
  "admin.pushClearAllErr": "מחיקת המנויים נכשלה. נסו שוב.",
  "admin.pushClearAllRedis": "Redis לא זמין — לא ניתן למחוק מנויים.",
  "admin.pushClearAllVerifyErr":
    "המחיקה לא אומתה (נשארו שדות ב־Redis). נסו שוב או בדקו במסוף Upstash/Vercel KV.",
  "admin.pushClearAllWorking": "מוחק…",
  "admin.pushServerMissingHint":
    "במכשיר הזה יש מנוי Push מקומי, אבל בשרת אין מנוי שמור — לחצו «עדכן רישום Push» או «שלח בדיקת Push».",
  "admin.pushTestBtn": "שלח בדיקת Push",
  "admin.pushTestWorking": "שולח בדיקה…",
  "admin.pushTestOk":
    "נשלחה הודעת בדיקה ל-{n} מכשיר/ים. אם לא קיבלתם — לחצו «עדכן רישום Push» ונסו שוב.",
  "admin.pushTestNoSubs":
    "אין מנוי Push בשרת. לחצו «עדכן רישום Push (מחדש)» במכשיר הזה.",
  "admin.pushTestStale":
    "המנוי בשרת היה לא תקף (למשל אחרי שינוי מפתחות VAPID) ונמחק. לחצו «עדכן רישום Push (מחדש)».",
  "admin.pushTestFail":
    "הבדיקה נכשלה לכל המנויים. לחצו «עדכן רישום Push (מחדש)».",
  "admin.pushTestErr": "שליחת בדיקת Push נכשלה. נסו שוב.",
  "admin.customerPushTitle": "הודעות ללקוחות (אפליקציה PWA)",
  "admin.customerPushHint":
    "שליחת Push לכל מי שהתקין את האתר כ-PWA ואישר התראות. אחרי התקנה או בפתיחת האפליקציה מוצג חלון «אפשר התראות» — רק מי שמאשר נרשם כמנוי.",
  "admin.customerPushStatusLine":
    "מנויי לקוחות PWA: ‏{count} · VAPID ‏{vapid} · Redis ‏{redis}",
  "admin.customerPushTitleLabel": "כותרת ההודעה",
  "admin.customerPushTitlePh": "למשל: מבצע סוף שבוע",
  "admin.customerPushBodyLabel": "תוכן ההודעה",
  "admin.customerPushBodyPh": "למשל: 10% הנחה על כל ההזמנה עד יום ראשון",
  "admin.customerPushUrlLabel": "קישור בלחיצה (אופציונלי)",
  "admin.customerPushUrlPh": "/",
  "admin.customerPushSendBtn": "שלח התראה לכל המנויים",
  "admin.customerPushSending": "שולח…",
  "admin.customerPushConfirm":
    "לשלוח התראה Push לכל הלקוחות שאישרו התראות באפליקציה?",
  "admin.customerPushOk": "נשלחה התראה ל-{n} מכשיר/ים.",
  "admin.customerPushPartial":
    "נשלח ל-{sent}, נכשל {failed}. הוסרו {removed} מנויים לא תקפים.",
  "admin.customerPushNoSubs":
    "אין עדיין מנויי Push. לקוחות עם PWA צריכים לפתוח את האפליקציה וללחוץ «אפשר התראות» בחלון שמופיע.",
  "admin.customerPushErr": "שליחת ההודעה נכשלה. נסו שוב.",
  "admin.customerPushInvalid": "יש למלא כותרת ותוכן.",
  "admin.ratingsTitle": "דירוגי לקוחות",
  "admin.ratingsHint":
    "צפייה בדירוגים שהלקוחות שלחו באתר או אחרי הזמנה. אפשר למחוק דירוג מסוים או את כל הדירוגים.",
  "admin.ratingsSummary": "ממוצע {avg} מתוך 5 · {count} דירוגים",
  "admin.ratingsRefresh": "רענן",
  "admin.ratingsLoading": "טוען דירוגים…",
  "admin.ratingsEmpty": "אין עדיין דירוגים.",
  "admin.ratingsRedis": "Redis לא זמין — לא ניתן לטעון דירוגים.",
  "admin.ratingsLoadErr": "טעינת הדירוגים נכשלה.",
  "admin.ratingsDelete": "מחק",
  "admin.ratingsDeleting": "מוחק…",
  "admin.ratingsDeleteOk": "הדירוג נמחק.",
  "admin.ratingsDeleteErr": "מחיקת הדירוג נכשלה. נסו שוב.",
  "admin.ratingsDeleteConfirm": "למחוק את הדירוג הזה?",
  "admin.ratingsDeleteAll": "מחק את כל הדירוגים",
  "admin.ratingsDeleteAllOk": "כל הדירוגים נמחקו.",
  "admin.ratingsDeleteAllConfirm":
    "למחוק את כל הדירוגים? פעולה זו תאפס את הממוצע וכמות הדירוגים באתר.",
  "admin.ratingsTypeOrder": "הזמנה",
  "admin.ratingsTypeGuest": "אתר",
  "admin.ratingsSourceSite": "דיאלוג כניסה",
  "admin.ratingsSourceSuccess": "אחרי הזמנה",
  "admin.ratingsSourceReminder": "תזכורת",
  "admin.ratingsNoComment": "ללא הערה",
  "admin.secretLabel": "קוד מנהל",
  "admin.secretPh": "סיסמה",
  "admin.load": "כניסה",
  "admin.loading": "טוען…",
  "admin.errConfig": "הגדרת מנהל חסרה בשרת.",
  "admin.errAuth": "סיסמה שגויה או שגיאת שרת.",
  "admin.errNet": "שגיאת רשת.",
  "admin.empty": "אין הזמנות עדיין.",
  "admin.payment": "תשלום",
  "admin.pattyPrepTitle": "סיכום קציצות להכנה (בורגרים)",
  "admin.pattyPrepLine": "{n}× קציצת {g} גר׳",
  "admin.pattyPrep600Note":
    "למנה 600 גר׳ (×{n} במנה): הספירה לעיל מבוססת על 3×200 גר׳ למנה; ניתן גם להרכיב 2×220 + 160 גר׳.",
  "admin.prepMealToppingsTitle": "סיכום תוספות למנות (בורגר / קריספי)",
  "admin.prepSideExtrasTitle": "סיכום תוספות רטבים בצד",
  "admin.prepAggLine": "{n}× {label}",
  "admin.orderId": "מזהה",
  "admin.daySalesTotal": "סה״כ מכירות מזון (ללא דמי משלוח)",
  "admin.monthSalesTotal": "סה״כ מכירות מזון בחודש (ללא דמי משלוח):",
  "admin.monthDeliveryFeesTotal": "סה״כ דמי משלוח בחודש:",
  "admin.dayDeliveryFeesTotal": "סה״כ דמי משלוח באותו יום",
  "admin.orderFoodAmountLabel": "מזון:",
  "admin.orderDeliveryAmountLabel": "משלוח:",
  "admin.salesCalendarTitle": "מכירות לפי יום",
  "admin.salesCalendarHint": "לפי תאריך בישראל (Asia/Jerusalem). לחצו על יום כדי לראות הזמנות.",
  "admin.siteVisitsTitle": "ביקורים יומיים — אתר ואפליקציה",
  "admin.siteVisitsHint":
    "ספירה של ביקור אחד ליום לכל מכשיר (כניסה ראשונה ביום מ־burgerhut.co.il או מהאפליקציה שהותקנה למסך הבית). לפי יום בישראל (ירושלים).",
  "admin.siteVisitsColDay": "יום",
  "admin.siteVisitsColTotal": "סה״כ",
  "admin.siteVisitsColWeb": "דפדפן",
  "admin.siteVisitsColPwa": "אפליקציה",
  "admin.siteVisitsRedisHint":
    "אין נתונים: Redis לא מוגדר בשרת (נדרש לאיסוף ביקורים, כמו לקופונים).",
  "admin.siteVisitsLoadErr": "לא ניתן לטעון את נתוני הביקורים.",
  "admin.pwaInstallsTotalTitle": "התקנות PWA מצטברות",
  "admin.pwaInstallsTotalHint":
    "מספר מכשירים שדווחו כהתקנה (הוספה למסך הבית): אירוע התקנה בכרום/אדג׳, וב־iOS — פתיחה ראשונה מהאייקון. לא כולל התקנות שלא נשלחו לשרת.",
  "admin.calendarPrevMonth": "חודש קודם",
  "admin.calendarNextMonth": "חודש הבא",
  "admin.calendarTodayBtn": "היום",
  "admin.dayOrdersEmpty": "אין הזמנות ביום זה.",
  "admin.ordersCount": "הזמנות",
  "admin.delete": "מחק הזמנה",
  "admin.deleteConfirm":
    "האם אתה בטוח שברצונך למחוק את ההזמנה? פעולה זו אינה ניתנת לביטול.",
  "admin.markDone": "בוצע",
  "admin.markingDone": "מסמן כבוצע…",
  "admin.doneStatus": "סטטוס: בוצעה",
  "admin.deleting": "מוחק…",
  "admin.deleteErr": "לא ניתן היה למחוק את ההזמנה.",
  "admin.doneErr": "לא ניתן היה לעדכן סטטוס להזמנה.",
  "admin.catalogTitle": "ניהול קטלוג — מנות ותוספות",
  "admin.catalogHint":
    "הוסיפו מנות חדשות (מזהה באנגלית קטנה ומקפים), ערכו מחיר/תמונה/שמות, או הסירו פריט מהתפריט (מנות מהמערכת מוסתרות ולא נמחקות לצמיתות). שמות ריקים בעריכת מנה קיימת = שימוש בטקסט המקורי מהאתר.",
  "admin.catalogSaving": "שומר קטלוג…",
  "admin.catalogErr": "לא ניתן היה לשמור את הקטלוג.",
  "admin.catalogSaved": "הקטלוג נשמר.",
  "admin.catalogAdd": "הוסף מנה",
  "admin.catalogEdit": "ערוך",
  "admin.catalogRemove": "הסר מהתפריט",
  "admin.catalogRemoveConfirm":
    "להסיר פריט זה מהתפריט הגלוי? (מנות מובנות ניתן להחזיר מִסעיף «מוסתרים».)",
  "admin.catalogHiddenTitle": "מנות מובנות מוסתרות",
  "admin.catalogRestore": "החזר לתפריט",
  "admin.catalogId": "מזהה (אנגלית, לדוגמה new-burger)",
  "admin.catalogCategory": "קטגוריה",
  "admin.catalogPrice": "מחיר (₪)",
  "admin.catalogImage": "תמונה",
  "admin.catalogImageUrlHint":
    "אפשר להדביק קישור לתמונה למטה, או לבחור קובץ וללחוץ «העלה». ההעלאה לענן עלולה לקחת כ־10–60 שניות ברשת נייד.",
  "admin.catalogImageUrlPaste": "קישור ישיר לתמונה (אופציונלי)",
  "admin.catalogImageUrlPasteHint":
    "אם יש לכם כתובת https של תמונה — אין חובה להעלות קובץ. אם אין Blob ב־Vercel, זו הדרך המומלצת.",
  "admin.catalogImageUploadWait":
    "מעלה לשרת… אל תסגרו את החלון. ברשת איטית זה עלול לקחת עד דקה. אם נכשל — בדקו ש־BLOB_READ_WRITE_TOKEN מוגדר ב־Vercel או הדביקו קישור לתמונה.",
  "admin.catalogImageRequired": "נדרש קישור לתמונה או תמונה שהועלתה בהצלחה לפני השמירה.",
  "admin.catalogUploadBtn": "העלה מהטלפון / מהמחשב",
  "admin.catalogImageUploading": "מעלה תמונה…",
  "admin.catalogImageUploaded": "התמונה הועלתה. שמרו את המנה כדי לשמור בקטלוג.",
  "admin.catalogNameHe": "שם בעברית",
  "admin.catalogNameAr": "שם בערבית",
  "admin.catalogDescHe": "תיאור בעברית (אופציונלי)",
  "admin.catalogDescAr": "תיאור בערבית (אופציונלי)",
  "admin.catalogSaveRow": "שמור",
  "admin.catalogCancel": "ביטול",
  "admin.catalogModalAdd": "מנה חדשה",
  "admin.catalogModalEdit": "עריכת מנה",
  "admin.catalogSlugHint":
    "רק אותיות קטנות, מספרים ומקף. לא לשכפל מזהה של מנה קיימת.",
  "admin.catalogErrId": "מזהה לא תקין או כבר קיים.",
  "admin.catalogErrIdDetail":
    "מזהה לא תקין: השתמשו רק באותיות קטנות באנגלית, מספרים ומקף (ללא רווחים). לדוגמה: loaded-burger-bowl. לתמונה: הדביקו קישור https או לחצו «העלה» אחרי בחירת קובץ.",
  "admin.catalogErrNames": "נדרשים שם בעברית ובערבית למנה חדשה.",
  "admin.catalogToppingsSectionTitle": "תוספות לבורגר ולקריספי",
  "admin.catalogToppingsHint":
    "רשימה זו מופיעה מעל קטגוריית «תוספות» (צ׳יפס וכו׳). תוספת מותאמת אישית יכולה להיות «בורגר בלבד» אם מסמנים שלא להציג במנות קריספי.",
  "admin.catalogToppingAdd": "הוסף תוספת",
  "admin.catalogToppingModalAdd": "תוספת חדשה",
  "admin.catalogToppingModalEdit": "עריכת תוספת",
  "admin.catalogToppingPrice": "מחיר (₪)",
  "admin.catalogToppingExcludeCrispy": "לא להציג במנות קריספי",
  "admin.catalogToppingBuiltinHint":
    "שמות ריקים משאירים את הטקסט מהתפריט. מחיר ותמונה נשמרים כהתאמה למנה המובנית.",
  "admin.catalogToppingCrispyYes": "גם בקריספי",
  "admin.catalogToppingCrispyNo": "בורגר בלבד",
  "admin.catalogToppingHiddenTitle": "תוספות מובנות מוסתרות",
  "admin.catalogToppingRemoveConfirm": "להסיר את התוספת מהתפריט?",
  "admin.menuExportSheetTitle": "תפריט",
  "admin.catalogExportPng": "הורד תמונה (PNG)",
  "admin.catalogExportPdf": "הורד PDF ‏(A4)",
  "admin.catalogExportBusy": "מייצא…",
  "admin.catalogExportHint":
    "הקובץ משקף את התפריט הגלוי בקטלוג (שפה כמו במסך הניהול). PNG מתאים לפרסום באינסטגרם; PDF נוח להדפסה או לשליחה כמסמך.",
  "admin.catalogExportErr": "הייצוא נכשל. נסו שוב או דפדפן אחר.",
  "admin.inventoryTitle": "ניהול מלאי — מנות ותוספות",
  "admin.inventoryHint":
    "סמנו פריט כשהוא זמין למכירה. בטלו סימון כשאזל מהמלאי (ברירת מחדל: הכול זמין). כולל תוספות לבורגר. מעקב קציצות (למטה) משפיע אוטומטית על זמינות בורגרים ומנכה בהזמנה.",
  "admin.inventoryPattySectionTitle": "מלאי קציצות (גרם)",
  "admin.inventoryPattyHint":
    "לאחר שמירה, בורגרים שלא ניתן להרכיב מהכמויות יסומנו אוטומטית כלא זמינים; כל הזמנה מנכה קציצות. כיבוי המעקב משבית ניכוי וחישוב אוטומטי.",
  "admin.inventoryPattyGramLabel": "קציצות {g} גר׳",
  "admin.inventoryPattySave": "שמור כמויות קציצות",
  "admin.inventoryPattyDisableTracking": "כבה מעקב קציצות",
  "admin.inventoryPattyAutoBadge": "לא זמין אוטומטית — מלאי קציצות",
  "admin.inventoryBurgerToppings": "תוספות לבורגר",
  "admin.inventorySalads": "סלטים (מלאי)",
  "admin.inventorySaving": "שומר מלאי…",
  "admin.inventoryErr": "לא ניתן היה לשמור את המלאי.",
  "admin.hoursTitle": "זמני פעילות המסעדה",
  "admin.hoursHint":
    "סמנו «פתוח» לכל יום שבו המסעדה פעילה, והגדירו שעת פתיחה וסגירה. מותר מעבר חצות: למשל פתיחה 22:00 וסגירה 02:00 — הסגירה בבוקר של היום הבא. הדיאלוג «זמני עבודה» באתר יציג את ההגדרות.",
  "admin.hoursOpen": "פתוח",
  "admin.hoursClosedReasonPlaceholder":
    "סיבת סגירה (תוצג ללקוחות מתחת להודעה בדף הבית)",
  "admin.hoursOpening": "שעת פתיחה",
  "admin.hoursClosing": "שעת סגירה",
  "admin.hoursSave": "שמור זמנים",
  "admin.hoursSaving": "שומר…",
  "admin.hoursSaved": "הזמנים נשמרו.",
  "admin.hoursErr": "לא ניתן היה לשמור את זמני העבודה.",
  "admin.hoursErrInvalidTime": "שעת פתיחה או סגירה לא תקינה. בדקו שכל יום פתוח כולל שעות מלאות.",
  "admin.hoursErrCloseBeforeOpen":
    "שעת פתיחה וסגירה לא יכולות להיות זהות. מעבר חצות מותר (למשל 22:00–02:00).",
  "admin.hoursErrInvalidDays": "נתוני הימים לא תקינים. רעננו את הדף ונסו שוב.",
  "admin.hoursErrStorage":
    "השרת לא הצליח לשמור (אחסון). ב-Vercel ודאו ש־KV מחובר וש־KV_REST_API_TOKEN הוא טוקן מלא, לא read-only בלבד.",
  "admin.discountTitle": "מבצעים והנחות",
  "admin.discountHint":
    "הגדירו הנחה באחוזים עבור הזמנות מעל סכום מסוים. ההנחה חלה על עלות המנות בלבד (ללא משלוח), וניתן להפעיל/לכבות בכל עת.",
  "admin.discountEnabled": "להפעיל הנחה",
  "admin.discountPercent": "אחוז הנחה (%)",
  "admin.discountMinOrder": "החל מסכום הזמנה (₪)",
  "admin.discountReason": "סיבת ההנחה",
  "admin.discountReasonPh": "למשל: מבצע סוף שבוע / פתיחה חגיגית",
  "admin.couponEnabled": "להפעיל קופונים ללקוחות",
  "admin.couponPercent": "אחוז נקודות לקופון (%)",
  "admin.discountSave": "שמור הנחה",
  "admin.discountSaving": "שומר הנחה…",
  "admin.discountSaved": "ההנחה נשמרה.",
  "admin.discountErr": "לא ניתן היה לשמור את ההנחה.",
  "admin.deliveryFeesTitle": "דמי משלוח לפי כפר",
  "admin.deliveryFeesHint":
    "קבעו את מחיר המשלוח לכל יישוב שמסומן במפה. המחיר חל רק על נקודה בתוך הגבול של אותו כפר.",
  "admin.deliveryFeesSave": "שמור מחירי משלוח",
  "admin.deliveryFeesSaving": "שומר מחירי משלוח…",
  "admin.deliveryFeesSaved": "מחירי המשלוח נשמרו.",
  "admin.deliveryFeesErr": "לא ניתן היה לשמור את מחירי המשלוח.",
  "admin.deliveryFeesErrInvalid": "מחיר משלוח חייב להיות מספר בין 0 ל־500.",
  "admin.couponsTitle": "קופונים",
  "admin.couponsHint": "כאן ניתן לראות את כל הקופונים שהופקו ולמחוק לפי צורך.",
  "admin.couponTotalsUnused":
    "סה״כ גובה הנחה — קופונים שלא נוצלו (בתוקף):",
  "admin.couponTotalsUsed": "סה״כ גובה הנחה — קופונים שכבר נוצלו:",
  "admin.couponBadgeLegend": "סימון:",
  "admin.couponFilterLabel": "סינון לפי מצב:",
  "admin.couponFilterEmpty": "אין קופונים במצב שנבחר.",
  "admin.couponsEmpty": "אין קופונים עדיין.",
  "admin.couponCode": "קוד קופון",
  "admin.couponValue": "גובה הנחה",
  "admin.couponExpiryStatus": "תוקף",
  "admin.couponUsedStatus": "נוצל",
  "admin.couponValid": "בתוקף",
  "admin.couponExpired": "פג תוקף",
  "admin.couponUsed": "נוצל",
  "admin.couponNotUsed": "לא נוצל",
  "admin.couponCreatedAt": "נוצר",
  "admin.couponExpiresAt": "פג תוקף ב",
  "admin.couponRewardFromOrder": "התקבל מהזמנה מס׳:",
  "admin.couponRedeemedTitle": "מימוש הקופון",
  "admin.couponUsedAt": "תאריך ושעת מימוש",
  "admin.couponUsedAtUnknown": "תאריך מימוש לא נשמר",
  "admin.couponRedeemedOrderNumber": "מספר הזמנה (מימוש)",
  "admin.couponRedeemedOrderUnknown": "לא נשמר",
  "admin.couponDelete": "מחיקת קופון",
  "admin.couponDeleteConfirm": "למחוק קופון זה מהמערכת?",
  "admin.couponDeleted": "הקופון נמחק.",
  "admin.couponDeleteErr": "לא ניתן היה למחוק את הקופון.",
  "admin.promoTitle": "פרסום מדיה",
  "admin.promoHint":
    "וידאו: העלה קובץ וידאו באיכות מקור. כש-Blob מחובר לפרויקט אין מגבלת ~4MB של פונקציית Vercel. בכניסה לאתר הסרטון יוצג במסך מלא; הלקוח יכול ללחוץ «דלג». אחרי דילוג באותו ביקור הסרטון לא יופיע שוב; סרטון חדש יוצג שוב אוטומטית. תמונות: סליידר אופקי בדף הבית בין בורגר 160 לבורגר 200 (גלילה משמאל לימין).",
  "admin.promoVideoSection": "סרטון בכניסה לאתר",
  "admin.sliderSection": "סליידר תמונות בדף הבית",
  "admin.sliderShowOnHome": "הצג סליידר בדף הבית ללקוחות",
  "admin.sliderShowOnHomeHint":
    "כבוי — התמונות לא ייטענו מהאחסון (חוסך Blob). הרשימה למטה נשארת לניהול; אפשר להפעיל שוב בכל עת.",
  "admin.sliderShowOnHomeHintFs":
    "כבוי — הסליידר מוסתר מלקוחות. התמונות עצמן נשארות בתיקיית public/home-slider/ בפרויקט.",
  "admin.sliderDisplaySaved": "הגדרת התצוגה נשמרה.",
  "admin.sliderHintFs":
    "מוצג בין בורגר 160 לבורגר 200. שמים עד 24 קבצי תמונה (jpg, png, webp, gif, avif) בתיקייה public/home-slider/, דוחפים ל-Git ומפרסמים — יוצגו אוטומטית לכולם, בלי Blob. אפשר גם להעלות מהטלפון מהאזור למטה (נשמר ב-KV או מקומית).",
  "admin.sliderKvHint":
    "העלאה מהטלפון: מיד עם בחירת התמונה מתחילה העלאה לשרת (JSON דחוס — עמיד יותר בנייד). אם ההעלאה נכשלה — לחצו על «העלה לסליידר מהטלפון» או השתמשו בקישור למטה. הקוד נשמר גם ב-localStorage. בפרודקשן חובה Redis/KV.",
  "admin.sliderUploadLitePageLink":
    "דף העלאה מצומצם לנייד (אם הדף מתרענן מהמצלמה)",
  "admin.sliderUploadLiteTitle": "העלאת תמונה לסליידר (מצומצם)",
  "admin.sliderUploadLiteHint":
    "דף קל — בחרו תמונה ואז לחצו «העלה». אין העלאה אוטומטית. הקוד נטען מהזיכרון אם שמרתם אותו ב«טען» בפאנל הראשי.",
  "admin.sliderUploadLiteBack": "← חזרה לפאנל ניהול",
  "admin.sliderUploadLiteNeedSecret": "הזינו קוד מנהל או טענו קודם בפאנל הראשי.",
  "admin.sliderUploadPhoneBtn": "העלה לסליידר מהטלפון",
  "admin.sliderUploadingKv": "מעלה…",
  "admin.sliderUploadedKv": "התמונה נוספה לסליידר (העלאה).",
  "admin.sliderUploadRequiresKv":
    "העלאה מהטלפון דורשת Redis/KV בפרודקשן. הגדירו Vercel KV בפרויקט או הוסיפו תמונות דרך public/home-slider/ + Git.",
  "admin.sliderUploadTooLarge": "הקובץ גדול מדי. נסו תמונה קטנה יותר.",
  "admin.sliderMaxImages": "הגעתם למקסימום תמונות בסליידר.",
  "admin.sliderUploadInvalidImage": "הקובץ אינו תמונה תקינה.",
  "admin.sliderUploadFailed": "ההעלאה נכשלה. נסו שוב.",
  "admin.sliderItemKindUpload": "תמונה מהעלאה (שרת)",
  "admin.sliderItemKindFs": "תמונה מהפרויקט (Git)",
  "admin.sliderDeleteKv": "מחק תמונה שהועלתה",
  "admin.sliderKvDeleteConfirm": "למחוק תמונה זו מהסליידר? (לא ניתן לשחזר)",
  "admin.sliderKvDeleted": "התמונה נמחקה.",
  "admin.sliderEmptyFs":
    "אין קבצים בתיקייה public/home-slider/. הוסיפו תמונות שם ושחררו גרסה.",
  "admin.sliderUploadBtn": "העלאת תמונה לסליידר",
  "admin.sliderUploading": "מעלה תמונה…",
  "admin.sliderUploaded": "התמונה נוספה לסליידר.",
  "admin.sliderEmpty": "אין תמונות בסליידר.",
  "admin.sliderDelete": "הסרה מהסליידר",
  "admin.sliderDeleteConfirm": "להסיר תמונה זו מהסליידר? (הקובץ יימחק מ־Blob אם אפשר)",
  "admin.sliderDeleted": "התמונה הוסרה מהסליידר.",
  "admin.sliderDeleteErr": "לא ניתן היה להסיר את התמונה.",
  "admin.sliderClearAll": "הסר את כל התמונות",
  "admin.sliderClearAllConfirm":
    "להסיר את כל תמונות הסליידר בבת אחת? (קבצי Blob יימחקו מהאחסון אם אפשר)",
  "admin.sliderClearedAll": "כל תמונות הסליידר הוסרו.",
  "admin.sliderClearAllErr": "לא ניתן היה להסיר את כל התמונות.",
  "admin.sliderPersistErr":
    "השמירה לשרת נכשלה (בדרך כלל חיבור KV/Redis). בדקו משתני סביבה; בלי שמירה תקינה השינויים לא יישמרו.",
  "admin.sliderErrMissing": "בחר קובץ תמונה לפני ההעלאה.",
  "home.sliderAria": "תמונות פרסום",
  "admin.promoStatusOn": "מוצג ללקוחות (כשיש קובץ)",
  "admin.promoStatusOff": "מושבת — לא מוצג ללקוחות",
  "admin.promoNoFile": "אין קובץ וידאו. בחר קובץ והעלה.",
  "admin.promoUploadBtn": "העלה סרטון",
  "admin.promoUploading": "מעלה…",
  "admin.promoUploaded": "הסרטון הועלה והופעל.",
  "admin.promoHomeDisplaySection": "הצגה בדף הבית",
  "admin.promoShowOnHomeCheckbox": "הצג את סרטון הפרסום בדף הבית (מסך מלא אוטומטי)",
  "admin.promoShowOnHomeHint":
    "בטלו את הסימון כדי להפסיק את הצגת הסרטון ללקוחות. הקובץ נשמר בשרת — אפשר להפעיל שוב מתי שתרצו.",
  "admin.promoSaving": "שומר…",
  "admin.promoSaved": "ההגדרה נשמרה.",
  "admin.promoErrUpload": "העלאה נכשלה.",
  "admin.promoErrInvalidType": "יש להעלות קובץ וידאו בלבד.",
  "admin.promoErrTooLarge": "הקובץ גדול מדי (מקסימום כ־80MB).",
  "admin.promoErrTooLargeVercel":
    "הקובץ גדול מדי להעלאה מהפאנל בפרודקשן (Vercel, בדרך כלל עד ~4MB). דחס/הקטן את הווידאו או העלה דרך אחסון חיצוני.",
  "admin.promoErrBlobConfig":
    "חסר חיבור ל-Vercel Blob (BLOB_READ_WRITE_TOKEN). חבר Blob לפרויקט ב-Vercel ואז נסה שוב.",
  "admin.promoErrBlobUpload":
    "העלאת Blob נכשלה בפרודקשן. בדוק שה-Blob מחובר לפרויקט הנכון, שהטוקן מוגדר ב-Production, ושבוצע Redeploy.",
  "admin.promoErrMissing": "בחר קובץ לפני ההעלאה.",
  "admin.promoErrSave": "לא ניתן היה לשמור את הקובץ.",
  "admin.promoLoadEmpty":
    "לא נטען מצב פרסום. סגרו והיכנסו שוב או רעננו את הדף.",

  "weekday.0": "יום א׳",
  "weekday.1": "יום ב׳",
  "weekday.2": "יום ג׳",
  "weekday.3": "יום ד׳",
  "weekday.4": "יום ה׳",
  "weekday.5": "יום ו׳",
  "weekday.6": "שבת",
};

const AR = {
  "header.tagline":
    "مطعم برجر وكرسبي — توصيلات واستلام ذاتي في يركا والمناطق المحيطة",
  "header.goHome": "الانتقال إلى الصفحة الرئيسية",
  "header.brandSecondary": "برجرهات",
  "header.admin": "إدارة",
  "lang.switch": "اللغة",
  "lang.he": "עברית",
  "lang.ar": "العربية",

  "home.title": "جائعون؟ اطلبوا الآن ونحضّر لكم بمحبة ❤",
  "home.titleLine1": "جائعون؟ اطلبوا الآن",
  "home.titleLine2": "ونحضّر لكم بمحبة",
  "home.burgerMascotAlt": "شخصية مرحة تأكل برجراً",
  "home.contactTitle": "التواصل والموقع",
  "home.contactCopyright": "© אקסטרים טכנולוגיות",
  "home.contactCopyrightEmail": "extremetechnologies22@gmail.com",
  "home.contactPhoneLabel": "الهاتف",
  "home.contactAddressLabel": "العنوان",
  "home.contactPhoneValue": "0504847599",
  "home.callPhoneAria": "اتصال بالمطعم",
  "home.contactAddressValue": "يركا 137",
  "home.instagramAria": "انستغرام المطعم",
  "home.tiktokAria": "تيك توك المطعم",
  "home.instagramUrl": "https://www.instagram.com/burger._.hut?igsh=c3UwZzliZDBtNXQz",
  "home.tiktokUrl": "https://www.tiktok.com/@burger._.hut?_r=1&_t=ZS-95A2cZnTFdV",
  "home.openMap": "فتح في الخرائط",
  "home.openWaze": "waze للمطعم",
  "home.adminDiscreetSr": "دخول لوحة المطعم",
  "home.termsTitle": "الشروط والأحكام",
  "home.termsLead":
    "سياسة إلغاء المعاملة وفق قانون حماية المستهلك الإسرائيلي.",
  "home.termsLawCitation":
    "إلغاء المعاملة وفق لوائح حماية المستهلك (إلغاء المعاملة)، 5770-2010 وقانون حماية المستهلك، 5741-1981",
  "home.hoursButton": "ساعات العمل",
  "home.restaurantOpen": "المطعم مفتوح",
  "home.restaurantClosed": "المطعم مغلق",
  "home.restaurantOpensAt16": "يفتح الساعة 16:00",
  "home.hoursDialogTitle": "ساعات العمل",
  "home.hoursDays": "من الاثنين إلى السبت",
  "home.hoursTime": "من الساعة 10:00 حتى وقت الإغلاق (حسب اليوم — انظر القائمة)",
  "home.hoursOrderWindow":
    "يمكن إرسال الطلبات من الساعة 10:00 بداية الدوام وفق أوقات العمل المعروضة",
  "home.hoursNoOpenDays": "لم يُعرّف أي يوم عمل.",
  "home.orderingClosedBannerOpenDay":
    "يفتح المطعم الساعة 16:00 — يمكن تقديم الطلب من الساعة 10:00 حتى وقت الإغلاق في ذلك اليوم.",
  "home.preOrderBanner":
    "يمكن إرسال الطلبات الآن. بداية العمل الساعة {openTime}",
  "home.preOpenDialogTitle": "تنبيه",
  "home.preOpenDialogLine1":
    "المطعم مغلق حالياً، يفتح الساعة {openTime}.",
  "home.preOpenDialogLine2":
    "يمكن تقديم الطلب. بداية العمل الساعة {openTime}.",
  "home.preOpenDialogAck": "فهمت",
  "home.orderingClosedBannerDayOff": "المطعم مغلق يوم {day}",
  "home.discountAnnounceWithReason":
    "بسبب {reason} يمكنكم الحصول على خصم بنسبة {percent}% عند الشراء فوق {min}.",
  "home.discountAnnounceNoReason":
    "يمكنكم الحصول على خصم بنسبة {percent}% عند الشراء فوق {min}.",
  "home.categoryBannerAria": "اختيار فئة القائمة",
  "home.promoSkip": "تخطي",
  "home.promoTapForSound": "اضغط لتشغيل الصوت",
  "home.promoAria": "إعلان فيديو في الصفحة الرئيسية",
  "pwa.iosInAppBrowserTitle": "فتح في Safari أو Chrome",
  "pwa.iosInAppShortLine1": "لتنزيل التطبيق على آيفونك،",
  "pwa.iosInAppShortPrefix": "ادخل الموقع من متصفح ",
  "pwa.iosInAppShortOr": " أو ",
  "pwa.iosInAppShortSuffix": ".",
  "pwa.iosInAppShortHint":
    "بعد فتح المتصفح: مشاركة (السهم لأعلى) ← «إضافة إلى الشاشة الرئيسية».",
  "pwa.iosOpenInSafariAria": "فتح الموقع في Safari",
  "pwa.iosOpenInChromeAria": "فتح الموقع في Chrome",
  "pwa.iosCopyLinkButton": "نسخ عنوان الموقع",
  "pwa.iosCopyLinkDone":
    "تم النسخ. افتح Safari، الصق في شريط العنوان، ثم مشاركة ← إضافة إلى الشاشة الرئيسية.",
  "pwa.iosCopyLinkFail": "فشل النسخ التلقائي. انسخ يدوياً:",
  "pwa.iosSafariGuideTitle": "إضافة من Safari",
  "pwa.iosGuideClose": "فهمت",
  "pwa.iosInstallHint":
    "في Safari على آيفون:\n\n• اضغط مشاركة.\n• مرّر للأسفل — «إضافة إلى الشاشة الرئيسية» غالباً تحت جهات الاتصال.\n• إن لم تظهر: «تعديل الإجراءات…» في الأسفل وفعّل الخيار.",
  "pwa.androidInstallHint":
    "إذا لم يظهر مربع التثبيت: اضغط ⋮ في قائمة Chrome واختر «تثبيت التطبيق» أو «إضافة إلى الشاشة الرئيسية».",
  "pwa.launcherAria": "إضافة إلى الشاشة الرئيسية — وصول أسرع",
  "pwa.launcherCaption": "حمّل التطبيق",

  "customerPush.bannerTitle": "عروض وتحديثات",
  "customerPush.bannerBody":
    "استلموا إشعارات عن العروض والخصومات والمنتجات الجديدة — إذا ثبّتم Burger Hut على الشاشة الرئيسية.",
  "customerPush.enableBtn": "نعم، أرسلوا لي التحديثات",
  "customerPush.dismissBtn": "ليس الآن",
  "customerPush.enabling": "جارٍ التفعيل…",
  "customerPush.permissionDenied": "الإشعارات محظورة. اسمحوا بها من إعدادات التطبيق/المتصفح.",
  "customerPush.unavailable": "الإشعارات غير متاحة على الخادم بعد.",
  "customerPush.subscribeErr": "تعذّر التسجيل الآن. حاولوا لاحقاً.",
  "pwaNotify.title": "احصلوا على عروض وخصومات",
  "pwaNotify.body":
    "ثبّتم Burger Hut — فعّلوا الإشعارات لتصلكم العروض والخصومات والأخبار من المطعم.",
  "pwaNotify.systemHint":
    "بعد «تفعيل الإشعارات» ستظهر نافذة من الهاتف — اختاروا «السماح».",
  "pwaNotify.enableBtn": "تفعيل الإشعارات",
  "pwaNotify.laterBtn": "لاحقاً",
  "pwaNotify.enabling": "جارٍ التفعيل…",
  "pwaNotify.permissionDenied":
    "الإشعارات محظورة. فعّلوها من إعدادات التطبيق/المتصفح لتصلكم العروض.",
  "pwaNotify.unavailable": "الإشعارات غير متاحة على الخادم بعد.",
  "pwaNotify.subscribeErr": "تعذّر التسجيل الآن. حاولوا لاحقاً.",
  "rating.title": "كيف كانت تجربتكم؟",
  "rating.subtitle": "قيّموا جودة الطعام في المطعم (1–5 نجوم)",
  "rating.starsAria": "تقييم بالنجوم",
  "rating.starN": "{n} نجوم",
  "rating.commentPh": "ملاحظة (اختياري)",
  "rating.submitBtn": "إرسال التقييم",
  "rating.submitting": "جارٍ الإرسال…",
  "rating.skipBtn": "ليس الآن",
  "rating.thanks": "شكراً على تقييمكم!",
  "rating.pickStars": "اختاروا عدد النجوم",
  "rating.submitErr": "فشل إرسال التقييم. حاولوا مرة أخرى.",
  "rating.unavailable": "التقييمات غير متاحة على الخادم بعد.",
  "rating.orderNotFound": "الطلب غير موجود.",
  "rating.headerCount": "{n} تقييم",
  "rating.headerAria": "متوسط التقييم {avg} من 5، {n} تقييم",
  "rating.headerOpenList": "عرض تقييمات العملاء",
  "rating.listTitle": "تقييمات العملاء",
  "rating.listLoading": "جارٍ تحميل التقييمات…",
  "rating.listEmpty": "لا توجد تقييمات بعد.",
  "rating.listClose": "إغلاق",
  "rating.anonymousCustomer": "عميل",
  "rating.starsOfFive": "{n} من 5",
  "rating.siteTitle": "قيّمونا",
  "rating.siteSubtitle": "نرحب بآرائكم عن طعام المطعم",
  "rating.siteNameLabel": "الاسم",
  "rating.siteNamePh": "اسمكم",
  "rating.siteNameRequired": "يجب تعبئة الاسم",
  "rating.siteSendBtn": "إرسال",
  "rating.siteAfterTasteBtn": "بعد أن أتذوق",
  "rating.siteNeverBtn": "لا تعرض مرة أخرى",
  "rating.reminderHint": "طلب #{order}",
  "pwa.confirmTitle": "إضافة Burger Hut إلى الشاشة الرئيسية؟",
  "pwa.confirmBody":
    "بضغطة واحدة يظهر أيقونة على هاتفك — طلب أسرع دون البحث عن الموقع من جديد.",
  "pwa.confirmInstall": "تثبيت",
  "pwa.confirmCancel": "ليس الآن",
  "cat.burgers": "برجر",
  "cat.specials": "مميز",
  "cat.crispy": "دجاج مقرمش",
  "cat.sides": "بطاطا وإضافات",
  "cat.drinks": "مشروبات",

  "menu.kids-burger-120.name": "برجر أطفال 120غ",
  "menu.kids-burger-120.desc": "وجبة أطفال — 120 غرام.",
  "menu.burger-160.name": "برجر 160غ",
  "menu.burger-160.desc": "برجر كلاسيكي — 160 غرام.",
  "menu.burger-200.name": "برجر 200غ",
  "menu.burger-200.desc": "برجر — 200 غرام.",
  "menu.burger-220.name": "برجر 220غ",
  "menu.burger-220.desc": "برجر — 220 غرام.",
  "menu.smash-burger-240.name": "برجر سماش 240غ",
  "menu.smash-burger-240.desc":
    "سماش — 240 غرام. أقراص البرجر في الوجبة: 2×120غ.",
  "menu.smash-burger-360.name": "برجر سماش 360غ",
  "menu.smash-burger-360.desc":
    "سماش — 360 غرام. أقراص البرجر في الوجبة: 3×120غ.",
  "menu.burger-320.name": "برجر 320غ",
  "menu.burger-320.desc":
    "برجر — 320 غرام. أقراص البرجر في الوجبة: 2×160غ.",
  "menu.burger-360.name": "برجر 360غ",
  "menu.burger-360.desc":
    "برجر — 360 غرام. أقراص البرجر في الوجبة: 200 + 160غ.",
  "menu.burger-400.name": "برجر 400غ",
  "menu.burger-400.desc":
    "برجر — 400 غرام. أقراص البرجر في الوجبة: 2×200غ.",
  "menu.burger-440.name": "برجر 440غ",
  "menu.burger-440.desc":
    "برجر — 440 غرام. أقراص البرجر في الوجبة: 2×220غ.",
  "menu.burger-480.name": "برجر 480غ",
  "menu.burger-480.desc":
    "برجر — 480 غرام. أقراص البرجر في الوجبة: 3×160غ.",
  "menu.burger-520.name": "برجر 520غ",
  "menu.burger-520.desc":
    "برجر — 520 غرام. أقراص البرجر في الوجبة: 200 + 160 + 160غ.",
  "menu.burger-560.name": "برجر 560غ",
  "menu.burger-560.desc":
    "برجر — 560 غرام. أقراص البرجر في الوجبة: 200 + 200 + 160غ.",
  "menu.burger-600.name": "برجر 600غ",
  "menu.burger-600.desc":
    "برجر — 600 غرام. أقراص البرجر في الوجبة: 3×200غ (أو بديل 2×220 + 160غ).",
  "menu.special-truffle-king.name": "Truffle King",
  "menu.special-truffle-king.desc":
    "قرص 200غ، جودا، فطر مقلي، أيولي ترفل، خس وبصل أحمر. نكهة غنية.",
  "menu.special-truffle-king.fixedMealToppings":
    "جبنة جودا، فطر مقلي، أيولي ترفل.",
  "menu.special-bbq-smoke.name": "BBQ Smoke",
  "menu.special-bbq-smoke.desc":
    "قرص 200غ، أسادو مفتت، شيدر، صلصة باربيكيو، بصل كراميل، مخلل وخس. مدخن وحلو.",
  "menu.special-bbq-smoke.fixedMealToppings":
    "أسادو مفتت، شيدر، باربيكيو، بصل كراميل.",
  "menu.special-fire-burger.name": "Fire Burger",
  "menu.special-fire-burger.desc":
    "قرص 200غ، شيدر، هالبينيو، بصل كراميل، مايونيز حار وفلفل حار، خس وطماطم. حار.",
  "menu.special-fire-burger.fixedMealToppings":
    "شيدر، هالبينيو، بصل كراميل، مايونيز حار، فلفل حار.",
  "menu.special-cheese-bomb.name": "Cheese Bomb",
  "menu.special-cheese-bomb.desc":
    "قرص 240غ محشو شيدر، جودا فوق، صلصة شيدر على الجانب، خس، طماطم، بصل أحمر ومخلل. جبن ذائب.",
  "menu.special-cheese-bomb.fixedMealToppings":
    "قرص 240غ، شيدر في الحشو، جودا فوق، صلصة شيدر على الجانب.",
  "menu.special-lamb-bacon-deluxe.name": "Lamb Bacon Deluxe",
  "menu.special-lamb-bacon-deluxe.desc":
    "قرص 200غ، بايكون غنم مقرمش، شيدر، بصل كراميل، خس، طماطم، مايونيز وباربيكيو خفيف.",
  "menu.special-lamb-bacon-deluxe.fixedMealToppings":
    "بايكون غنم مقرمش، شيدر، بصل كراميل، مايونيز، باربيكيو خفيف.",
  "menu.special-corned-beef-stack.name": "Corned Beef Stack",
  "menu.special-corned-beef-stack.desc":
    "قرص 200غ، كورند بيف عجل، جودا، مخلل، بصل أحمر، خس وصلصة خردل بالعسل ومايونيز خفيف. أسلوب ديلي.",
  "menu.special-corned-beef-stack.fixedMealToppings":
    "كورند بيف عجل، جودا، مايونيز خفيف.",
  "menu.special-lettuce-burger.name": "برجر بالخس",
  "menu.special-lettuce-burger.desc":
    "برجر 160غ حسب الطلب — خس بدل الخبز، صلصات وإضافات وسلطات حسب الاختيار. يبدأ من 55 ₪.",
  "menu.special-lettuce-burger.fixedMealToppings":
    "قرص لحم 160غ، أوراق خس بدل الخبز؛ صلصات وإضافات وسلطات حسب الاختيار.",
  "menu.crispy-chicken-burger.name": "كريسبي تشيكن (مدورة/بدون مدورة)",
  "menu.crispy-chicken-burger.desc": "كريسبي — بخبز مدور أو بدون.",
  "menu.crispy-chicken-burger.lineNameRound": "كريسبي تشيكن مدورة",
  "menu.crispy-chicken-burger.lineNameNoRound": "كريسبي تشيكن بدون مدورة",
  "menu.crispy-chicken-burger-kids.name":
    "كريسبي تشيكن أطفال (مدورة/تورتيلا صغيرة)",
  "menu.crispy-chicken-burger-kids.desc": "كريسبي — مدورة أو تورتيلا.",
  "menu.crispy-chicken-burger-kids.lineNameRoundSmall":
    "كريسبي تشيكن أطفال — خبز مدور صغير",
  "menu.crispy-chicken-burger-kids.lineNameSmallTortilla":
    "كريسبي تشيكن أطفال — تورتيلا صغيرة",
  "menu.crispy-chicken-burger-kids.lineNameNoBread":
    "كريسبي تشيكن أطفال — بدون خبز",
  "menu.crispy-chicken-tortilla-large.name": "كريسبي تشيكن تورتيلا كبيرة",
  "menu.crispy-chicken-tortilla-large.desc": "تورتيلا بدجاج مقرمش — حجم كبير.",
  "menu.side-fries.name": "بطاطا",
  "menu.side-fries.desc": "بطاطا متموجة.",
  "menu.side-sweet-potato.name": "بطاطا حلوة",
  "menu.side-sweet-potato.desc": "",
  "menu.side-onion-rings.name": "حلقات بصل",
  "menu.side-onion-rings.desc": "",
  "menu.side-mashed-balls.name": "كرات بوريه",
  "menu.side-mashed-balls.desc": "",
  "menu.side-mix.name": "ميكس",
  "menu.side-mix.desc": "بطاطا متموجة، بطاطا حلوة، حلقات بصل وكرات بوريه.",
  "menu.side-chips-cheddar-symphony.name": "بطاطا مع صلصة شيدر",
  "menu.side-mozzarella-sticks.name": "أصابع موزاريلا",
  "menu.side-mozzarella-sticks.desc": "٧ أصابع موزاريلا.",
  "menu.side-home-fries.name": "هوم فرايز",
  "menu.side-home-fries.desc": "مقلية بصلصة تشيلي حلوة.",
  "menu.side-home-fries-spicy.name": "هوم فرايز حار",
  "menu.side-home-fries-spicy.desc": "مقلية بصلصة تشيلي حلوة وحارة.",
  "menu.side-chips-cheddar-symphony.desc":
    "بطاطا متموجة مع صلصة جبنة شيدر.",
  "menu.drink-cola.name": "كوكا كولا",
  "menu.drink-cola.desc": "",
  "menu.drink-zero.name": "كولا زيرو",
  "menu.drink-zero.desc": "",
  "menu.drink-sprite.name": "سبرايت",
  "menu.drink-sprite.desc": "",
  "menu.drink-fanta.name": "فانتا",
  "menu.drink-fanta.desc": "",
  "menu.drink-tapuzina-grape.name": "تبوزينا عنب",
  "menu.drink-tapuzina-grape.desc": "",
  "menu.drink-strawberry-banana.name": "تبوزينا فراولة موز",
  "menu.drink-strawberry-banana.desc": "",
  "menu.drink-xl.name": "XL",
  "menu.drink-xl.desc": "مشروب طاقة، علبة 250 مل.",
  "menu.drink-xl-ten.name": "XL TEN",
  "menu.drink-xl-ten.desc":
    "مشروب طاقة فوّار بدون سكر، علبة 250 مل (10 سعرات).",
  "menu.drink-blue-mojito.name": "BLUE موهيتو",
  "menu.drink-blue-mojito.desc": "مشروب طاقة BLUE بنكهة موهيتو.",
  "menu.drink-soda.name": "صودا",
  "menu.drink-soda.desc": "Schweppes — مياه صودا فوّارة.",
  "menu.drink-water-small.name": "ماي عدن",
  "menu.drink-water-small.desc": "",
  "menu.drink-fresh-strawberry.name": "فريش فراولة",
  "menu.drink-fresh-strawberry.desc":
    "Fresher — فراولة، مياه معدنية طبيعية وفيتامينات.",
  "menu.drink-cola-15l.name": "كولا / زيرو 1.5 لتر",
  "menu.drink-cola-15l.desc": "",
  "menu.drink-zero-15l.name": "كوكا كولا زيرو 1.5 لتر",
  "menu.drink-zero-15l.desc": "",

  "topping.cheddar": "جبنة شيدر",
  "topping.gouda": "جبنة جودا",
  "topping.fried_egg": "بيضة عين",
  "topping.jalapeno": "هالابينيو",
  "topping.caramel_mushrooms": "فطر مكرمل",
  "topping.caramel_onion": "بصل مكرمل",
  "topping.avocado": "أفوكادو",
  "topping.lamb_bacon": "بيكون عبور",
  "topping.veal_corned": "كورندبيف عجل",
  "topping.pulled_asado": "أسادو مفكك",

  "salad.salad_none": "بدون سلطة",
  "salad.salad_lettuce": "خس",
  "salad.salad_tomato": "بندورة",
  "salad.salad_pickles": "خيار مكبوس",
  "salad.salad_onion": "بصل",
  "salad.salad_coleslaw": "سلطة كول سلو",

  "sauce.sauce_house_regular": "صلصة البيت عادية",
  "sauce.sauce_house_spicy": "صلصة البيت حارة",
  "sauce.sauce_bbq": "باربيكيو",
  "sauce.sauce_sweet_chili": "تشيلي حلو",
  "sauce.sauce_hot_chili": "تشيلي حار",
  "sauce.sauce_garlic": "ثوم",
  "sauce.sauce_cheddar": "صلصة شيدر",

  "ui.freeSalads": "سلطة",
  "ui.adultCrispyNoRound": "بدون خبز مدور",
  "ui.kidsCrispyBread.title": "مدورة صغيرة / تورتيلا صغيرة / بدون خبز",
  "ui.kidsCrispyBread.round": "مدورة صغيرة",
  "ui.kidsCrispyBread.small_tortilla": "تورتيلا صغيرة",
  "ui.kidsCrispyBread.none": "بدون خبز",
  "ui.saucePricingHint":
    "اختر صلصة واحدة دون تكلفة. من الصلصة الثانية 4₪ لكل صلصة. صلصة الشيدر 8₪.",
  "ui.saucePlus": "+₪",
  "ui.saladOutOfStock": "(نفد من المخزون)",
  "ui.menuPattyRemainingFew": "تبقّى {n} وجبات",
  "ui.sauceAddOne": "إضافة حصة صلصة",
  "ui.sauceRemoveOne": "تقليل حصة صلصة",
  "ui.doubleWord": "دبل",
  "ui.bunSauceOnBunTitle": "صلصة على الخبز؟",
  "ui.bunSauceYes": "نعم",
  "ui.bunSauceNo": "لا",
  "ui.donenessTitle": "درجة التسوية",
  "ui.doneness.rare": "Rare",
  "ui.doneness.medium": "Medium",
  "ui.doneness.medium_well": "Medium well",
  "ui.doneness.well_done": "Well done",
  "ui.burgerToppings": "إضافات البرجر",
  "ui.crispyToppings": "إضافات الكرسبي",
  "ui.extraSauces": "صلصات إضافية على الجانب",
  "ui.addToCart": "أضف إلى السلة",
  "ui.updateMealInCart": "تحديث في السلة",
  "ui.pattyInsufficientForMeal":
    "من هذا النوع بقي بالمخزون (مع السلة): {count}.\nخفّضوا الكمية أو اختاروا طبقاً آخر.",
  "ui.pattyInsufficientForMealFallback":
    "لا يمكن إضافة هذه الكمية حسب مخزون الأقراص والسلة. خفّضوا الكمية أو اختاروا طبقاً آخر.",
  "ui.pattyInsufficientCapOne":
    "من هذا النوع يتوفر طبق واحد فقط حسب المخزون. خفّضوا الكمية إلى 1 أو اختاروا طبقاً آخر.",
  "ui.pattyInsufficientCapMany":
    "من هذا النوع يمكن طلب {max} كحد أقصى حسب المخزون. خفّضوا الكمية إلى {max} أو اختاروا طبقاً آخر.",
  "ui.pattyInsufficientTypeExhausted":
    "من هذا النوع لا يتوفر مخزون حسب الأقراص (مع باقي السلة). اختاروا طبقاً آخر أو خفّضوا كمية في سلة أخرى.",
  "ui.pattyStockCheckFailed":
    "تعذّر التحقق من مخزون الأقراص الآن. حاولوا مرة أخرى بعد لحظة.",
  "ui.pattyCartHavePattyZero": "لا توجد أقراص {g} غرام في المخزون",
  "ui.pattyCartHavePattyOne": "قرص واحد {g} غرام",
  "ui.pattyCartHavePattyMany": "{have} أقراص {g} غرام",
  "ui.pattyCartBumpShort":
    "{stock}. تحاول إضافة المزيد من نوع «{name}» — غير ممكن، اختر مנה أخرى.",
  "ui.pattyCartUnknownItem": "الطبق",
  "ui.added": "تمت الإضافة",
  "ui.mealValidateTitle": "خيارات ناقصة",
  "ui.mealValidateSalads": "لم تختَر السلطات.",
  "ui.mealValidateSauces":
    "لم تختَر صلصات إضافية على الجانب، لك صلصة واحدة مجاناً.",
  "ui.mealValidateFries": "لم تختر مقليات الوجبة. (إلزامي الاختيار)",
  "ui.mealValidateAddAnyway": "أنا أعرف — أضف للسلة",
  "ui.mealValidateGoBack": "العودة للاختيار",
  "ui.outOfStock": "لا يتوفر هذا الطبق اليوم",
  "ui.soldOutShort": "نفد",
  "ui.openCustomize": "اختيار السلطات والإضافات والصلصات والمقليات",
  "ui.editComponents": "تعديل المكونات",
  "ui.fromPrice": "ابتداءً من",
  "ui.wizardClose": "إغلاق",
  "ui.wizardStep": "خطوة",
  "ui.wizardNext": "التالي",
  "ui.wizardBack": "رجوع",
  "ui.wizardCancel": "إلغاء",
  "ui.wizardStepAdd": "الكمية والإضافة للسلة",
  "ui.wizardServingTitle": "خيارات التقديم",
  "ui.wizardPriceLine": "سعر الوحدة (حسب اختيارك):",
  "ui.wizardAllOnOneScreen":
    "كل الخيارات أدناه — مرّروا واختاروا، ثم أضيفوا للسلة في الأسفل.",
  "ui.wizardSpecialFullCustomizeHint":
    "مكوّنات الوجبة حسب الوصفة معروضة أدناه. يمكن أيضاً إضافة إضافات للبرجر وتعديل السلطات ودرجة النضج والصلصات الجانبية والمقليات والمشروب — مرّروا وأضيفوا للسلة في الأسفل.",
  "ui.wizardSpecialSaladsOnly":
    "إضافات الوجبة (جبن، ما يأتي مع الوجبة…) ثابتة حسب الوصفة. في الوجبة قرص واحد — تختارون وزناً 200 أو 220غ أدناه. يمكن أيضاً تعديل السلطات، درجة النضج، الصلصات الجانبية والمشروب — مرّروا وأضيفوا للسلة في الأسفل.",
  "ui.wizardSpecialSaladsOnlyCheeseBomb":
    "إضافات الوجبة ثابتة حسب الوصفة. قرص 240غ. يمكن تعديل السلطات ودرجة النضج والصلصات الجانبية والمشروب — مرّروا وأضيفوا للسلة في الأسفل.",
  "ui.specialPattyTitle": "وزن القرص",
  "ui.specialPatty200": "200غ",
  "ui.specialPatty220": "220غ (+₪5)",
  "ui.specialPatty200Depleted": "200غ — نفد",
  "ui.specialPatty220Depleted": "220غ (+₪5) — نفد",
  "ui.specialFixedToppingsTitle": "إضافات في الوجبة",
  "ui.specialFixedToppingsHint": "لا يمكن تغييرها",
  "ui.mealFriesForMealLabel": "اختر المقليات مع الوجبة:",
  "ui.mealFriesSelectPlaceholder": "اختر",
  "ui.mealFriesMultiHint": "يمكن اختيار أكثر من نوع معاً.",
  "ui.mealFriesValidate": "اختر خياراً واحداً على الأقل للمقليات مع الوجبة قبل الإضافة إلى السلة.",
  "ui.mealFries.none": "بدون مقليات",
  "ui.mealFries.regular": "بطاطا",
  "ui.mealFries.mix": "ميكس البيت",
  "ui.mealFries.onion-rings": "حلقات بصل",
  "ui.mealFries.potato-balls": "كرات بطاطا مهروسة",
  "ui.mealFries.potato": "بطاطا حلوة",
  "ui.mealFries.chips-cheddar-symphony": "سيمفونية بطاطا وجبنة شيدر",
  "ui.mealFries.mozzarella-sticks": "أصابع موزاريلا",
  "ui.mealFries.home-fries": "هوم فرايز",
  "ui.mealFries.home-fries-spicy": "هوم فرايز حار",
  "ui.addDrinkQuestion": "إضافة مشروب؟",
  "ui.addDrinkSelectPlaceholder": "اختروا مشروباً من القائمة",
  "ui.sellerNotes": "ملاحظات للبائع",
  "ui.sellerNotesPh": "مثلاً: بدون صلصات، تغليف منفصل…",

  "cart.label": "السلة",
  "cart.items": "منتجات",
  "cart.unitsOne": "منتج واحد",
  "cart.unitsMany": "{count} منتجات",
  "cart.checkout": "عرض السلة والدفع",
  "cart.showCart": "عرض السلة",
  "cart.hideCart": "إخفاء السلة",
  "cart.panelTitle": "سلتك",
  "cart.closePanel": "إغلاق",
  "cart.editMeal": "تعديل",
  "cart.menuStillVisibleHint":
    "يمكنك التمرير لأعلى — القائمة والتصنيفات لا تزال متاحة.",

  "checkout.title": "مراجعة الطلب والدفع",
  "checkout.backToMenu": "العودة إلى القائمة",
  "checkout.subtitle": "تأكد من التفاصيل ثم اختر طريقة الدفع المفضلة.",
  "checkout.emptyCart": "السلة فارغة. قم بإضافة منتجات من القائمة أولا.",
  "checkout.summary": "ملخص السلة",
  "checkout.size": "الحجم",
  "checkout.variant": "التقديم",
  "checkout.saladsPrefix": "سلطة",
  "checkout.noSalads": "بدون سلطة",
  "checkout.specialMealComponentsPrefix": "المكوّنات",
  "checkout.bunSauceOnBunPrefix": "صلصة على الخبز",
  "checkout.donenessPrefix": "درجة التسوية",
  "checkout.toppingsPrefix": "إضافات البرجر",
  "checkout.extrasPrefix": "صلصات إضافية على الجانب",
  "checkout.mealFriesPrefix": "مقليات الوجبة",
  "checkout.mealFriesCartSummaryTitle": "ملخص مقليات الوجبات",
  "checkout.unitPrice": "سعر الوحدة",
  "checkout.remove": "حذف",
  "checkout.grandTotal": "المجموع الكلي",
  "checkout.customer": "تفاصيل العميل",
  "checkout.firstName": "الاسم الأول",
  "checkout.lastName": "اسم العائلة",
  "checkout.firstNamePh": "اكتب الاسم الأول",
  "checkout.lastNamePh": "اكتب اسم العائلة",
  "checkout.phone": "رقم الهاتف",
  "checkout.saveContactQuestion": "هل نحفظ بيانات العميل للمرة القادمة؟",
  "checkout.saveContactYes": "نعم",
  "checkout.saveContactNo": "لا",
  "checkout.emailOptional": "البريد (للفاتورة)",
  "checkout.emailPlaceholder": "مثال: name@example.com",
  "checkout.orderType": "نوع الطلب",
  "checkout.delivery": "توصيل",
  "checkout.pickup": "استلام من المحل",
  "checkout.address": "العنوان الكامل",
  "checkout.addressPh": "المدينة، الشارع، رقم البيت، ملاحظات للوصل",
  "checkout.payment": "طريقة الدفع",
  "checkout.restaurantClosedNow": "المطعم مغلق حالياً.",
  "checkout.restaurantClosedHoursHint":
    "اطّلع على ساعات العمل في الصفحة الرئيسية",
  "checkout.submit": "متابعة",
  "checkout.submitting": "جارٍ معالجة الطلب...",
  "checkout.cardHypNotConfigured":
    "الدفع بالبطاقة غير مهيأ على الخادم. تواصلوا مع المتجر أو اختاروا كاش أو Bit. (لمالك الموقع: أكملوا متغيرات Hyp في .env.local أو Vercel.)",
  "checkout.cardHypMissingLine": "ناقص على الخادم",
  "checkout.cardMissingSiteUrl":
    "لا يوجد عنوان الموقع لإعادة التوجيه بعد الدفع. عيّنوا NEXT_PUBLIC_SITE_URL أو SITE_URL في .env.local.",
  "checkout.cardPaymentInitFailed":
    "تعذر فتح صفحة الدفع. أعيدوا المحاولة أو اختاروا طريقة دفع أخرى.",
  "checkout.pickupBranch": "استلام من المحل",
  "checkout.deliveryWhere": "منطقة التوصيل",
  "checkout.zoneYarka": "يركا",
  "checkout.zoneOutside": "خارج يركا",
  "checkout.addressYarkaHint":
    "عنوان دقيق في يركا أو إعطاء التفاصيل للساعي هاتفياً",
  "checkout.addressOutsideHint":
    "عنوان التوصيل - المدينة، الشارع..",
  "checkout.calcDelivery": "احسب أجور التوصيل (مسار الطرق)",
  "checkout.geocoding": "جارٍ تحديد العنوان…",
  "checkout.geocodeFail": "لم يُعثر على العنوان. جرّب تفاصيل أوضح.",
  "checkout.deliveryFeeLine": "أجور التوصيل",
  "checkout.foodSubtotal": "مجموع الأصناف",
  "checkout.discountLine": "خصم",
  "checkout.couponTitle": "كوبون",
  "checkout.couponPh": "هل لديكم كوبون؟ أدخلوا الرمز هنا…",
  "checkout.couponApply": "فحص",
  "checkout.couponChecking": "جارٍ الفحص…",
  "checkout.couponApplied": "الكوبون صالح وتم تطبيقه.",
  "checkout.couponInvalid": "رمز الكوبون غير صالح.",
  "checkout.couponExpired": "رمز الكوبون منتهي الصلاحية.",
  "checkout.couponUsed": "رمز الكوبون مستخدم سابقاً.",
  "checkout.couponInvalidSubmit":
    "رمز الكوبون غير صالح. حدّث/أزل الكوبون ثم أعد المحاولة.",
  "checkout.couponExpiredSubmit":
    "رمز الكوبون منتهي الصلاحية. حدّث/أزل الكوبون ثم أعد المحاولة.",
  "checkout.couponUsedSubmit":
    "رمز الكوبون مستخدم سابقاً. حدّث/أزل الكوبون ثم أعد المحاولة.",
  "checkout.couponLine": "كوبون",
  "checkout.deliveryPayHow": "دفع أجور التوصيل",
  "checkout.payRestaurantAll": "الدفع للمطعم (الطعام + التوصيل)",
  "checkout.payCourierDeliveryFoodPrefix": "الدفع للمطعم ثمن الطعام (",
  "checkout.payCourierDeliveryFoodSuffix":
    ") والدفع للساعي أجور التوصيل نقداً",
  "checkout.deliveryPayCashTitle": "الدفع نقداً (توصيل)",
  "checkout.cashCourierFullHint":
    "عند الدفع نقداً تدفع للساعي ثمن الطعام وأجور التوصيل معاً — نقداً عند الاستلام.",
  "checkout.cashCourierTotalLabel": "المجموع للدفع للساعي",
  "checkout.deliveryPayOnlineTitle": "بطاقة / Bit — تقسيم دفع التوصيل",
  "checkout.deliveryPayOnlineHint":
    "قبل إتمام الدفع اختر: هل تدفع أونلاين المبلغ كاملاً (طعام + توصيل) أم الطعام فقط وتدفع أجور التوصيل نقداً للساعي.",
  "checkout.onlinePayFoodOnlyLabel": "مبلغ السحب بالبطاقة أو Bit (الطعام فقط):",
  "checkout.onlinePayDeliveryToCourier":
    "أجور التوصيل تُدفع نقداً للساعي.",
  "checkout.onlinePayFullLabel": "مبلغ السحب بالبطاقة أو Bit (طعام + توصيل):",
  "checkout.payCourierCashFull": "نقداً للساعي — طعام + توصيل",
  "checkout.distanceKm": "مسافة من المطعم",
  "checkout.drivingDistanceKm": "مسافة قيادة تقريبية (طرق)",
  "checkout.airFallbackNote":
    "لم يُعثر على مسار بالطرق — تم الاحتساب بالمسافة الجوية. حاول توضيح العنوان.",
  "checkout.addressPickHint": "اختر عنواناً من القائمة لدقة أفضل:",
  "checkout.mapOpenField": "اضغط لفتح الخريطة واختيار نقطة التوصيل",
  "checkout.mapChange": "تغيير الموقع على الخريطة",
  "checkout.mapTitle": "اختيار العنوان على الخريطة",
  "checkout.pickLocation": "اختر الموقع",
  "checkout.mapTapHint":
    "حرّكوا الخريطة أو اضغطوا «اختر موقعي». الحدود الملوّنة هي مناطق التوصيل: يركا ₪25، جولس ₪30، أبو سنان ₪35، كفر ياسيف ₪40، جت ₪35، يانوح ₪40.",
  "checkout.deliveryVillageLine": "البلدة",
  "checkout.outOfVillageTitle": "خارج منطقة التوصيل المسعّرة",
  "checkout.outOfVillageBody": "اتصلوا بالمطعم للاستفسار عن تكلفة التوصيل",
  "checkout.outOfVillageCall": "الاتصال بالمطعم",
  "checkout.outOfVillageClose": "إغلاق",
  "checkout.outOfVillagePayCash":
    "أنا على علم بتكلفة التوصيل وسأدفع نقداً للساعي",
  "checkout.deliveryFeeAgreedValue": "متفق عليها مع المطعم",
  "checkout.agreedDeliveryPayHint":
    "أجور التوصيل تُدفع نقداً للساعي حسب الاتفاق مع المطعم. مبلغ الطعام حسب وسيلة الدفع المختارة.",
  "checkout.mapUseMyLocation": "اختر موقعي",
  "checkout.mapLocating": "جارٍ التحديد… إذا ظهر طلب في الأعلى اضغطوا سماح.",
  "checkout.mapLocateDenied":
    "كروم حجب الموقع عن {host} (GPS الهاتف قد يكون شغّالاً). اضغطوا القفل بجانب العنوان → إعدادات الموقع → الموقع → سماح، ثم اضغطوا مرة أخرى.",
  "checkout.mapLocateUnavailable":
    "تعذّر الحصول على الموقع. تأكدوا أن خدمة الموقع مفعّلة ثم حاولوا مرة أخرى.",
  "checkout.mapLocateTimeout": "انتهى وقت تحديد الموقع. حاولوا مرة أخرى.",
  "checkout.mapLocateUnsupported": "المتصفح لا يدعم خدمة الموقع.",
  "checkout.mapStreet": "خريطة",
  "checkout.mapSatellite": "قمر صناعي",
  "checkout.mapConfirm": "تأكيد وتطبيق الموقع",
  "checkout.mapCancel": "إلغاء",
  "checkout.mapApplying": "جارٍ حساب التوصيل…",
  "checkout.mapAddressFallback":
    "نقطة من الخريطة — اذكر الشارع والرقم والملاحظات في الحقل أدناه",
  "checkout.mapSelectHint": "اختر نقطة التوصيل على الخريطة لحساب أجور التوصيل.",
  "checkout.outsideNotesOptional": "ملاحظات للتوصيل (اختياري)",
  "checkout.outsideNotesPh": "طابق، مدخل، تعليمات للساعي…",
  "checkout.mapErrBounds": "النقطة خارج منطقة الخدمة. اختر موقعاً داخل إسرائيل.",
  "checkout.mapErrFee": "تعذر حساب أجور التوصيل لهذه النقطة.",
  "checkout.mapErrGeneric": "تعذر حساب التوصيل. جرّب نقطة أخرى.",
  "checkout.mapErrNetwork": "خطأ في الشبكة. تحقق من الاتصال وحاول مرة أخرى.",
  "checkout.mapErrBadCoords":
    "نقطة غير صالحة — اضغطوا على الخريطة مرة أخرى أو حرّكوا قليلاً.",
  "checkout.mapErrServer": "خطأ في الخادم عند حساب التوصيل. أعيدوا المحاولة لاحقاً.",
  "checkout.mapErrBadQuery": "طلب غير صالح — حدّثوا الصفحة وحاولوا من جديد.",

  "err.firstName": "الاسم الأول مطلوب",
  "err.lastName": "اسم العائلة مطلوب",
  "err.phone": "الهاتف مطلوب",
  "err.address": "العنوان مطلوب للتوصيل (خارج يركا)",
  "err.mapRequired": "يجب اختيار نقطة التوصيل على الخريطة",
  "err.deliveryZone": "اختر منطقة التوصيل (يركا / خارج يركا)",
  "err.deliveryGeocode": "يُرجى تأكيد الموقع على الخريطة (لم يكتمل حساب التوصيل)",
  "err.deliveryPayTo":
    "اختر بالبطاقة/Bit: دفع كامل (طعام+توصيل) أو الطعام فقط وأجور التوصيل نقداً للساعي",
  "err.cart": "السلة فارغة",
  "err.unavailable":
    "في السلة صنف نفد من المخزون — احذفوه أو خفّضوا الكمية.",
  "err.insufficientPatties":
    "لا يوجد ما يكفي من أقراص البرجر لإتمام الطلب — عدّلوا السلة أو حاولوا لاحقاً.",
  "err.insufficientPattiesIntro": "لا يمكن إتمام الطلب — نفاد في المخزون:",
  "err.insufficientPattiesGram": "أقراص {g} غ (مطلوب {need}، متوفر {have})",
  "err.insufficientPattiesGramSep": " · ",
  "err.insufficientPattiesMealsIntro": "الأصناف ذات الصلة في السلة:",
  "err.insufficientPattiesMealItem": "×{qty} {name}",
  "err.insufficientPattiesMealSep": "، ",
  "err.insufficientPattiesAction":
    "عدّلوا السلة (احذفوا صنفاً أو خفّضوا الكمية) أو أعيدوا المحاولة لاحقاً.",
  "err.orderingClosed":
    "يمكن الطلب من الساعة 10:00 حتى وقت الإغلاق في ذلك اليوم. تبدأ معالجة الطلبات الساعة 16:00",

  "payment.cash": "كاش",
  "payment.card": "بطاقة ائتمان",
  "payment.bit": "Bit",
  "payment.backToCheckout": "العودة لاختيار طريقة الدفع",

  "card.title": "الدفع بالبطاقة",
  "card.subtitle":
    "يمكنكم العودة للسلة واختيار كاش أو Bit، أو المتابعة لبوابة الدفع.",
  "card.amountLabel": "المبلغ للخصم",
  "card.orderRef": "مرجع الطلب",
  "card.continueExternal": "متابعة الدفع ببطاقة الائتمان",
  "card.openingPayment": "جارٍ فتح صفحة الدفع…",
  "card.paymentStartFailed": "تعذر الاتصال ببوابة الدفع. أعيدوا المحاولة أو اختاروا طريقة أخرى.",
  "card.errorTechnical": "تفاصيل تقنية",
  "card.errorHypCode": "رمز الاستجابة من البوابة",
  "card.hintDnsNotFound":
    "مشكلة شبكة/DNS للوصول إلى Hyp Pay (pay.hyp.co.il). تحققوا من الإنترنت أو من HYP_PAY_BASE إن كان قد تغيّر.",
  "card.errNoSession":
    "لا توجد بيانات دفع. ارجعوا للسلة وأكملوا الطلب من جديد.",

  "success.paymentCompleted": "تم الدفع بنجاح 🎉",
  "success.paymentCompletedSub": "شكراً! تم تسجيل الطلب.",
  "success.titleOnline": "تم توجيهك لبوابة الدفع",
  "success.titleOk": "تم استلام طلبك!",
  "success.descOnline":
    "في البيئة الحقيقية سيتم تحويلك الآن إلى PayMe / Tranzila لإتمام الدفع.",
  "success.descBit": "شكراً! تم تسجيل الطلب في النظام.",
  "success.descCash":
    "شكراً! تم تسجيل الطلب في النظام. إذا فتح واتساب — راجِع الرسالة وأرسلها للمتجر.",
  "success.descCard":
    "شكراً! تم استلام الدفع وتسجيل الطلب.",
  "success.invoiceSentEmailAndSms":
    "أُرسلت الفاتورة إلى البريد الإلكتروني ورقم الهاتف المدخلين في الطلب (عند تفعيل الفوترة وSMS في الطرفية).",
  "success.invoiceSentEmailOnly":
    "أُرسلت الفاتورة إلى البريد الإلكتروني المدخل في الطلب (عند تفعيل وحدة الفوترة في الطرفية).",
  "success.invoiceSentSmsOnly":
    "أُرسلت الفاتورة عبر SMS إلى رقم الهاتف المدخل في الطلب (عند تفعيل الفوترة وحزمة SMS في الطرفية).",
  "success.invoiceCheckoutHintNoChannel":
    "لم تُرسل فاتورة من البوابة: أدخلوا في الطلب بريداً إلكترونياً صالحاً (حقل البريد الاختياري) أو رقم جوال إسرائيلي صالحاً لـ SMS. بدون ذلك لا يُرسل الموقع للبوابة طلب إصدار فاتورة.",
  "success.invoiceHypNoReceipt":
    "طلب الموقع فاتورة وفق بياناتكم، لكن عنوان العودة من البوابة لا يحتوي رقم مستند (Hesh). تحققوا من تفعيل وحدة الفوترة والبريد في طرفية Hyp، والوارد والبريد المزعج. مبلغ صغير (مثل 1 ₪) لا يمنع ذلك في كود الموقع.",
  "success.waSendOrder": "أرسل إلى واتساب",
  "success.waCashHeadline": "تم تسجيل طلبك في النظام",
  "success.waCashSubline": "أرسله لتنسيق وقت التحضير والاستلام",
  "success.waCardHeadline": "رائع! لدينا طلبيتك",
  "success.waCardNowLine": "الآن:",
  "success.waOrderRecordedWithId": "طلبك #{order} جاهز للإرسال",
  "success.waOrderRecordedNoId": "طلبك جاهز للإرسال",
  "success.loadingProgressAria": "جارٍ التحميل، يرجى الانتظار",
  "success.couponTitle": "🎁 حصلت على كوبون!",
  "success.couponValue": "💰 خصم ₪{value}",
  "success.couponCode": "🔢 الرمز: {code}",
  "success.couponExpiry": "⏳ صالح حتى: {date}",
  "success.couponRedeemSite": "للاستفادة ادخل إلى burgerhut.co.il",
  "success.couponCopy": "نسخ الرمز",
  "success.couponCopied": "تم نسخ الرمز",
  "success.couponScreenshotHint":
    "التقط الشاشة لحفظ الكوبون لاستخدامه في الطلب القادم!",
  "success.back": "العودة للقائمة",

  "cancel.title": "تم إلغاء الدفع",
  "cancel.desc":
    "لم يُخصم مبلغ. يمكنكم العودة لإكمال الطلب بطريقة دفع أخرى.",
  "cancel.backMenu": "العودة للقائمة",
  "cancel.backCheckout": "العودة للدفع",

  "paymentError.title": "لم يكتمل الدفع",
  "paymentError.desc":
    "حدث خطأ أثناء الدفع. إما لم يُخصم مبلغ أو رُفضت العملية. أعيدوا المحاولة أو اختاروا طريقة أخرى.",
  "paymentError.backMenu": "العودة للقائمة",
  "paymentError.backCheckout": "العودة للدفع",

  "bit.title": "الدفع عبر Bit",
  "bit.subtitle": "ادفع من هاتف الزبون عبر تطبيق Bit (يجب أن يكون مثبتاً).",
  "bit.payIntro":
    "هذا رقمنا لاستلام Bit: 0504847599\nاحفظوه في هاتفكم — Burger Hut\nستوا المبلغ — تكلفة الطلب: ₪{amount}",
  "bit.payForOrderLine": "الدفع عن الطلب رقم: {orderNumber}",
  "bit.copyNumber": "نسخ الرقم",
  "bit.copyAmount": "نسخ المبلغ",
  "bit.openApp": "فتح تطبيق Bit الآن",
  "bit.paidCheckbox": "دفعت عبر Bit",
  "bit.errNotifyAdmin": "تعذر إرسال الطلب للإدارة. أعيدوا المحاولة.",
  "bit.waOpening": "جارٍ الفتح…",
  "bit.errNoSession":
    "لا توجد بيانات طلب. ارجع إلى السلة وأكمل الطلب من جديد.",
  "bit.errBadJson": "بيانات الطلب غير صالحة.",
  "bit.errEmptyCart": "سلة الطلب فارغة في البيانات المحفوظة.",
  "bit.tryOpen": "نحاول فتح تطبيق Bit…",
  "bit.openFallback":
    "إذا لم يفتح Bit، قم بالدفع يدوياً ثم أرسل الطلب على واتساب.",
  "bit.copied": "تم النسخ",
  "bit.copyFail": "لم نتمكن من النسخ تلقائياً — انسخ يدوياً.",

  "float.wa": "التواصل عبر واتساب",

  "wa.brand": "*Burger Hut برجرهات*",
  "wa.newOrder": "طلب جديد:",
  "wa.orderNumber": "رقم الطلب",
  "wa.name": "الاسم",
  "wa.phone": "هاتف",
  "wa.address": "العنوان",
  "wa.orderType": "نوع الطلب",
  "wa.pickup": "استلام من المحل",
  "wa.delivery": "توصيل",
  "wa.payment": "طريقة الدفع",
  "wa.details": "تفاصيل الطلب",
  "wa.nonMealItemsHeader": "إضافات، بطاطا ومشروبات:",
  "wa.toppings": "إضافات البرجر",
  "wa.toppingsCrispy": "إضافات الكريسبي",
  "wa.salads": "سلطة",
  "wa.bunSauceOnBun": "صلصة على الخبز",
  "wa.doneness": "درجة التسوية",
  "wa.specialPatty": "وزن القرص",
  "wa.cheeseBombPattyLine": "240غ",
  "wa.gramsUnit": " غ",
  "wa.sauces": "صلصات إضافية على الجانب",
  "wa.linePrice": "السعر",
  "wa.drink": "مشروب",
  "wa.mealFries": "مقليات الوجبة",
  "wa.sellerNotes": "ملاحظات للبائع",
  "wa.total": "المجموع الكلي",
  "wa.foodSubtotal": "مجموع الأصناف",
  "wa.discount": "خصم العرض",
  "wa.coupon": "كوبون",
  "wa.couponDiscountLabel": "خصم الكوبون",
  "wa.couponCodeLabel": "رمز الكوبون",
  "wa.nisCurrencySuffix": "₪",
  "wa.totalWithDelivery": "المجموع مع التوصيل",
  "wa.deliveryArea": "منطقة التوصيل",
  "wa.zoneYarka": "يركا",
  "wa.zoneOutside": "خارج يركا",
  "wa.airDistance": "مسافة جوية (تقريبية)",
  "wa.drivingDistance": "مسافة قيادة تقريبية (طرق)",
  "wa.deliveryDistanceApprox": "مسافة تقريبية",
  "wa.deliveryVillage": "بلدة التوصيل",
  "wa.deliveryFeeAgreed": "متفق عليها مع المطعم",
  "wa.deliveryFee": "أجور التوصيل",
  "wa.payRestaurantInclDelivery": "الدفع للمطعم شاملاً أجور التوصيل",
  "wa.payCourierAgreed":
    "أجور التوصيل نقداً للساعي — متفق عليها مع المطعم",
  "wa.payCourierCashFull":
    "نقداً للساعي: المبلغ كاملاً (طعام + توصيل) — عند الزيارة",

  "admin.title": "إدارة المطعم — Burger Hut",
  "admin.backHome": "العودة للصفحة الرئيسية",
  "admin.hint": "صفحة داخلية للمشرف فقط. لا تشارك الرابط أو الرمز.",
  "admin.employeeBadge": "وضع الموظف",
  "admin.employeeHint":
    "وضع الموظف — المخزون، ساعات العمل، طلبات اليوم ومبيعات أمس (بدون أرشيف كامل).",
  "admin.employeeOrdersTitle": "الطلبات والمبيعات",
  "admin.employeeTodayBtn": "اليوم",
  "admin.employeeYesterdayBtn": "أمس",
  "admin.employeeAutoRefreshHint":
    "تُحدَّث طلبات اليوم وأمس تلقائياً كل نحو 15 ثانية عندما تبقى الصفحة مفتوحة.",
  "admin.secretStorageHint":
    "بعد «تحميل» يُحفظ رمز الإدارة فقط في المتصفح/الجهاز (ليس على الخادم). زائر آخر لا يرى الرمز — إلا إذا استخدم نفس الجهاز الذي دخلت منه. على جهاز مشترك اضغطوا «تسجيل الخروج» عند الانتهاء.",
  "admin.logoutBtn": "تسجيل الخروج وحذف الرمز من الجهاز",
  "admin.closeStore": "إغلاق المتجر",
  "admin.openStore": "فتح المتجر",
  "admin.storeCloseOutsideHours":
    "يمكن إغلاق المتجر أو فتحه فقط خلال ساعات العمل.",
  "admin.storeCloseErr": "تعذر تحديث حالة المتجر.",
  "admin.logoutConfirm":
    "حذف رمز الإدارة من المتصفح وإغلاق اللوحة؟ (ستحتاجون لإدخال الرمز مجدداً لاحقاً)",
  "admin.newOrderAutoRefreshHint":
    "تُحدَّث الطلبات والمبيعات اليومية تلقائياً كل نحو 15 ثانية عندما تبقى الصفحة مفتوحة.",
  "admin.newOrderNotifyHint":
    "بالضغط تُفعَّل الإشعارات على الشاشة والصوت والاهتزاز، وPush عند إغلاق التطبيق — إذا ضُبطت مفاتيح VAPID وRedis على الخادم.",
  "admin.newOrderNotifyEnable": "تفعيل الإشعارات",
  "admin.newOrderNotifyDenied":
    "الإشعارات محظورة في إعدادات المتصفح. يمكن السماح من إعدادات الموقع.",
  "admin.newOrderNotifyDeniedHelp":
    "Chrome / Edge (كمبيوتر): أيقونة القفل أو «i» بجانب العنوان → «إعدادات الموقع» / Site settings → الإشعارات → «السماح». ثم حدّثوا الصفحة. أندرويد (Chrome): ⋮ → الإعدادات → إعدادات الموقع → الإشعارات؛ أو إعدادات الجهاز → التطبيقات → Chrome → الإشعارات. آيفون (موقع أو PWA من الشاشة الرئيسية): الإعدادات → الإشعارات → اختيار التطبيق أو Safari/المتصفح → السماح؛ قد يظهر اسم الموقع كتطبيق مثبّت (iOS 16.4+ لـ Web Push في PWA).",
  "admin.newOrderNotifyTitle": "طلب جديد — Burger Hut",
  "admin.newOrderNotifyBodyOne": "وصل الطلب رقم #{n}.",
  "admin.newOrderNotifyBodyNoNum": "وصل طلب جديد.",
  "admin.newOrderNotifyCouponDiscount": " خصم الكوبون: {amount} ₪.",
  "admin.newOrderNotifyCouponCode": " رمز الكوبون: {code}.",
  "admin.newOrderNotifyBodyMany": "وصلت {c} طلبات جديدة.",
  "admin.pushSubscribeOk":
    "تم تفعيل إشعارات Push — ستصلكم إشعار حتى عند إغلاق التطبيق (إن سمح الجهاز والمتصفح).",
  "admin.pushSubscribeSkip":
    "إشعارات أثناء فتح الصفحة فقط. Push في الخلفية غير مضبوط على الخادم (VAPID) أو لا يوجد Redis.",
  "admin.pushSubscribeErr": "تعذّر تسجيل Push. حاولوا مرة أخرى.",
  "admin.pushSubscribeRedis": "Redis متصل لكن فشل حفظ الاشتراك.",
  "admin.pushSubscribeClientId":
    "تعذّر حفظ مُعرّف الجهاز (مثلاً المتصفح يحظر التخزين المحلي). اسمحوا بالتخزين للموقع أو جرّبوا متصفحاً آخر.",
  "admin.newOrderNotifyGrantedHint":
    "إشعارات الشاشة مفعّلة. على الجوال يُفضّل تسجيل Push لاستلام إشعار عند إغلاق التطبيق.",
  "admin.pushRegisterBtn": "تسجيل Push في الخلفية",
  "admin.pushReregisterBtn": "تحديث تسجيل Push (من جديد)",
  "admin.pushLocalActive":
    "هذا الجهاز مسجّل مسبقاً لـ Push — لا حاجة للضغط مجدداً في كل مرة. إذا غيّرتم المفاتيح على الخادم أو لا تصل الإشعارات، اضغطوا «تحديث التسجيل».",
  "admin.pushCheckingLocal": "جارٍ التحقق من تسجيل Push على الجهاز…",
  "admin.pushPerDeviceHint":
    "مهم: كل جهاز على حدة — للهاتف يلزم «تسجيل Push» من الهاتف. الرقم أعلاه = كم عنوان Push مختلف سيستلم إشعاراً. إعادة التسجيل تزيل التكرار لنفس الجهاز. إن بقي الرقم مرتفعاً: احذفوا كل الاشتراكات ثم سجّلوا فقط على الأجهزة المطلوبة.",
  "admin.pushStatusLine":
    "حالة خادم إشعارات الخلفية: VAPID ‏{vapid} · Redis ‏{redis} · أهداف Push فريدة (أجهزة): ‏{count}.",
  "admin.pushStatusOn": "مضبوط",
  "admin.pushStatusOff": "غير مضبوط",
  "admin.pushClearAllBtn": "حذف كل اشتراكات Push من الخادم",
  "admin.pushClearAllConfirm":
    "حذف كل اشتراكات الإشعارات (Push) المحفوظة على الخادم؟ بعد الحذف يلزم إعادة «تسجيل Push في الخلفية» على كل جهاز يحتاج إشعارات.",
  "admin.pushClearAllOk": "تم حذف كل الاشتراكات. أعيدوا تسجيل Push على الأجهزة المطلوبة.",
  "admin.pushClearAllErr": "فشل حذف الاشتراكات. حاولوا مرة أخرى.",
  "admin.pushClearAllRedis": "Redis غير متاح — لا يمكن الحذف.",
  "admin.pushClearAllVerifyErr":
    "لم يُؤكَّد الحذف (بقيت حقول في Redis). حاولوا مرة أخرى أو راجعوا Upstash/Vercel KV.",
  "admin.pushClearAllWorking": "جارٍ الحذف…",
  "admin.pushServerMissingHint":
    "يوجد اشتراك Push محلي على هذا الجهاز، لكن لا يوجد اشتراك محفوظ على الخادم — اضغطوا «تحديث تسجيل Push» أو «إرسال اختبار Push».",
  "admin.pushTestBtn": "إرسال اختبار Push",
  "admin.pushTestWorking": "جارٍ إرسال الاختبار…",
  "admin.pushTestOk":
    "أُرسلت رسالة اختبار إلى {n} جهاز/أجهزة. إن لم تصل — حدّثوا التسجيل وحاولوا مجدداً.",
  "admin.pushTestNoSubs":
    "لا يوجد اشتراك Push على الخادم. اضغطوا «تحديث تسجيل Push (من جديد)» على هذا الجهاز.",
  "admin.pushTestStale":
    "كان الاشتراك على الخادم غير صالح (مثلاً بعد تغيير مفاتيح VAPID) وحُذف. حدّثوا التسجيل.",
  "admin.pushTestFail":
    "فشل الاختبار لجميع الاشتراكات. اضغطوا «تحديث تسجيل Push (من جديد)».",
  "admin.pushTestErr": "فشل إرسال اختبار Push. حاولوا مرة أخرى.",
  "admin.customerPushTitle": "إشعارات للعملاء (تطبيق PWA)",
  "admin.customerPushHint":
    "إرسال Push لمن ثبّت الموقع كـ PWA ووافق على الإشعارات. بعد التثبيت أو عند فتح التطبيق يظهر «تفعيل الإشعارات» — فقط من يوافق يُسجَّل كمشترك.",
  "admin.customerPushStatusLine":
    "مشتركو PWA: ‏{count} · VAPID ‏{vapid} · Redis ‏{redis}",
  "admin.customerPushTitleLabel": "عنوان الرسالة",
  "admin.customerPushTitlePh": "مثال: عرض نهاية الأسبوع",
  "admin.customerPushBodyLabel": "نص الرسالة",
  "admin.customerPushBodyPh": "مثال: 10% خصم على الطلب حتى الأحد",
  "admin.customerPushUrlLabel": "رابط عند الضغط (اختياري)",
  "admin.customerPushUrlPh": "/",
  "admin.customerPushSendBtn": "إرسال إشعار لكل المشتركين",
  "admin.customerPushSending": "جارٍ الإرسال…",
  "admin.customerPushConfirm":
    "إرسال Push لكل العملاء الذين وافقوا على الإشعارات في التطبيق؟",
  "admin.customerPushOk": "تم الإرسال إلى {n} جهاز/أجهزة.",
  "admin.customerPushPartial":
    "أُرسل إلى {sent}، فشل {failed}. أُزيل {removed} اشتراك غير صالح.",
  "admin.customerPushNoSubs":
    "لا يوجد مشتركون Push بعد. عملاء PWA يفتحون التطبيق ويضغطون «تفعيل الإشعارات» في النافذة التي تظهر.",
  "admin.customerPushErr": "فشل إرسال الرسالة. حاولوا مرة أخرى.",
  "admin.customerPushInvalid": "يجب تعبئة العنوان والنص.",
  "admin.ratingsTitle": "تقييمات العملاء",
  "admin.ratingsHint":
    "عرض التقييمات التي أرسلها العملاء من الموقع أو بعد الطلب. يمكن حذف تقييم محدد أو كل التقييمات.",
  "admin.ratingsSummary": "المعدل {avg} من 5 · {count} تقييم",
  "admin.ratingsRefresh": "تحديث",
  "admin.ratingsLoading": "جارٍ تحميل التقييمات…",
  "admin.ratingsEmpty": "لا توجد تقييمات بعد.",
  "admin.ratingsRedis": "Redis غير متاح — لا يمكن تحميل التقييمات.",
  "admin.ratingsLoadErr": "فشل تحميل التقييمات.",
  "admin.ratingsDelete": "حذف",
  "admin.ratingsDeleting": "جارٍ الحذف…",
  "admin.ratingsDeleteOk": "تم حذف التقييم.",
  "admin.ratingsDeleteErr": "فشل حذف التقييم. حاولوا مرة أخرى.",
  "admin.ratingsDeleteConfirm": "حذف هذا التقييم؟",
  "admin.ratingsDeleteAll": "حذف كل التقييمات",
  "admin.ratingsDeleteAllOk": "تم حذف كل التقييمات.",
  "admin.ratingsDeleteAllConfirm":
    "حذف كل التقييمات؟ سيؤدي ذلك إلى تصفير المعدل وعدد التقييمات في الموقع.",
  "admin.ratingsTypeOrder": "طلب",
  "admin.ratingsTypeGuest": "موقع",
  "admin.ratingsSourceSite": "نافذة الدخول",
  "admin.ratingsSourceSuccess": "بعد الطلب",
  "admin.ratingsSourceReminder": "تذكير",
  "admin.ratingsNoComment": "بدون ملاحظة",
  "admin.secretLabel": "رمز المشرف",
  "admin.secretPh": "كلمة المرور",
  "admin.load": "دخول",
  "admin.loading": "جارٍ التحميل…",
  "admin.errConfig": "إعداد المسؤول غير مضبوط على الخادم.",
  "admin.errAuth": "رمز خاطئ أو خطأ في الخادم.",
  "admin.errNet": "خطأ في الشبكة.",
  "admin.empty": "لا توجد طلبات بعد.",
  "admin.payment": "الدفع",
  "admin.pattyPrepTitle": "ملخص الأقراص للتحضير (برجر)",
  "admin.pattyPrepLine": "{n}× قطعة {g} غ",
  "admin.pattyPrep600Note":
    "وجبة 600غ (×{n} في الطلبية): العد أعلاه يفترض 3×200غ لكل وجبة؛ يمكن أيضاً 2×220 + 160غ.",
  "admin.prepMealToppingsTitle": "ملخص الإضافات على الوجبات (برجر / كرسبي)",
  "admin.prepSideExtrasTitle": "ملخص صلصات إضافية على الجانب",
  "admin.prepAggLine": "{n}× {label}",
  "admin.orderId": "المعرّف",
  "admin.daySalesTotal": "إجمالي مبيعات الطعام (بدون أجور التوصيل)",
  "admin.monthSalesTotal": "إجمالي مبيعات الطعام في الشهر المعروض (بدون التوصيل):",
  "admin.monthDeliveryFeesTotal": "إجمالي أجور التوصيل في الشهر:",
  "admin.dayDeliveryFeesTotal": "إجمالي أجور التوصيل في ذلك اليوم",
  "admin.orderFoodAmountLabel": "طعام:",
  "admin.orderDeliveryAmountLabel": "توصيل:",
  "admin.salesCalendarTitle": "المبيعات حسب اليوم",
  "admin.salesCalendarHint": "حسب التاريخ في إسرائيل (Asia/Jerusalem). انقر يوماً لعرض الطلبات.",
  "admin.siteVisitsTitle": "زيارات يومية — الموقع والتطبيق",
  "admin.siteVisitsHint":
    "يُحتسب زيارة واحدة يومياً لكل جهاز (أول دخول في اليوم من الموقع أو من التطبيق المثبّت على الشاشة الرئيسية). حسب اليوم في إسرائيل (القدس).",
  "admin.siteVisitsColDay": "اليوم",
  "admin.siteVisitsColTotal": "الإجمالي",
  "admin.siteVisitsColWeb": "متصفح",
  "admin.siteVisitsColPwa": "تطبيق",
  "admin.siteVisitsRedisHint":
    "لا توجد بيانات: Redis غير مُعرّف على الخادم (مطلوب لجمع الزيارات، مثل الكوبونات).",
  "admin.siteVisitsLoadErr": "تعذّر تحميل بيانات الزيارات.",
  "admin.pwaInstallsTotalTitle": "إجمالي تثبيتات PWA",
  "admin.pwaInstallsTotalHint":
    "عدد الأجهزة التي أُبلغ عن تثبيتها (إضافة للشاشة الرئيسية): حدث التثبيت في كروم/إيدج، وفي iOS — أول فتح من الأيقونة. لا يشمل تثبيتات لم تُرسل للخادم.",
  "admin.calendarPrevMonth": "الشهر السابق",
  "admin.calendarNextMonth": "الشهر التالي",
  "admin.calendarTodayBtn": "اليوم",
  "admin.dayOrdersEmpty": "لا توجد طلبات في هذا اليوم.",
  "admin.ordersCount": "طلبات",
  "admin.delete": "حذف الطلب",
  "admin.deleteConfirm":
    "هل أنت متأكد أنك تريد حذف هذا الطلب؟ لا يمكن التراجع.",
  "admin.markDone": "تم التنفيذ",
  "admin.markingDone": "جارٍ تعليمه كمكتمل…",
  "admin.doneStatus": "الحالة: مكتمل",
  "admin.deleting": "جارٍ الحذف…",
  "admin.deleteErr": "تعذّر حذف الطلب.",
  "admin.doneErr": "تعذّر تحديث حالة الطلب.",
  "admin.catalogTitle": "إدارة الكتالوج — أطباق وإضافات",
  "admin.catalogHint":
    "أضف أطباقاً جديدة (معرّف إنجليزي صغير وشرطات)، عدّل السعر/الصورة/الأسماء، أو أزل عنصراً من القائمة الظاهرة (الأطباق المدمجة تُخفى ولا تُحذف نهائياً). أسماء فارغة عند تعديل طبق موجود = النص الأصلي من الموقع.",
  "admin.catalogSaving": "جارٍ حفظ الكتالوج…",
  "admin.catalogErr": "تعذّر حفظ الكتالوج.",
  "admin.catalogSaved": "تم حفظ الكتالوج.",
  "admin.catalogAdd": "إضافة طبق",
  "admin.catalogEdit": "تعديل",
  "admin.catalogRemove": "إزالة من القائمة",
  "admin.catalogRemoveConfirm":
    "إزالة هذا العنصر من القائمة الظاهرة؟ (يمكن إرجاع الأطباق المدمجة من قسم «المخفية».)",
  "admin.catalogHiddenTitle": "أطباق مدمجة مخفية",
  "admin.catalogRestore": "إرجاع للقائمة",
  "admin.catalogId": "المعرّف (إنجليزي، مثال new-burger)",
  "admin.catalogCategory": "الفئة",
  "admin.catalogPrice": "السعر (₪)",
  "admin.catalogImage": "صورة",
  "admin.catalogImageUrlHint":
    "يمكن لصق رابط الصورة أدناه، أو اختيار ملف ثم «رفع». الرفع للسحابة قد يستغرق نحو 10–60 ثانية على شبكة الجوال.",
  "admin.catalogImageUrlPaste": "رابط مباشر للصورة (اختياري)",
  "admin.catalogImageUrlPasteHint":
    "إذا لديكم عنوان https لصورة — لا يلزم رفع ملف. إذا لم يُضبط Blob في Vercel، هذا الخيار الأنسب.",
  "admin.catalogImageUploadWait":
    "جارٍ الرفع للخادم… لا تغلقوا النافذة. على شبكة بطيئة قد يستغرق ذلك حتى دقيقة. إذا فشل — تحققوا من BLOB_READ_WRITE_TOKEN في Vercel أو الصقوا رابط صورة.",
  "admin.catalogImageRequired": "يلزم رابط صورة أو صورة رُفعت بنجاح قبل الحفظ.",
  "admin.catalogUploadBtn": "رفع من الهاتف / الجهاز",
  "admin.catalogImageUploading": "جارٍ رفع الصورة…",
  "admin.catalogImageUploaded": "تم رفع الصورة. احفظوا الطبق لحفظه في الكتالوج.",
  "admin.catalogNameHe": "الاسم بالعبرية",
  "admin.catalogNameAr": "الاسم بالعربية",
  "admin.catalogDescHe": "وصف بالعبرية (اختياري)",
  "admin.catalogDescAr": "وصف بالعربية (اختياري)",
  "admin.catalogSaveRow": "حفظ",
  "admin.catalogCancel": "إلغاء",
  "admin.catalogModalAdd": "طبق جديد",
  "admin.catalogModalEdit": "تعديل طبق",
  "admin.catalogSlugHint":
    "أحرف صغيرة وأرقام وشرطة فقط. لا تكرّر معرّف طبق موجود.",
  "admin.catalogErrId": "معرّف غير صالح أو موجود مسبقاً.",
  "admin.catalogErrIdDetail":
    "معرّف غير صالح: استخدموا أحرفاً إنجليزية صغيرة وأرقاماً وشرطة فقط (بلا مسافات). مثال: loaded-burger-bowl. للصورة: الصقوا رابط https أو اضغطوا «رفع» بعد اختيار ملف.",
  "admin.catalogErrNames": "يلزم اسم بالعبرية واسم بالعربية للطبق الجديد.",
  "admin.catalogToppingsSectionTitle": "إضافات البرغر والكرسبي",
  "admin.catalogToppingsHint":
    "تظهر هذه القائمة فوق فئة «إضافات» (بطاطا وغيرها). يمكن جعل الإضافة المخصّصة «برغر فقط» عند عدم عرضها في وجبات الكرسبي.",
  "admin.catalogToppingAdd": "إضافة جديدة",
  "admin.catalogToppingModalAdd": "إضافة جديدة",
  "admin.catalogToppingModalEdit": "تعديل إضافة",
  "admin.catalogToppingPrice": "السعر (₪)",
  "admin.catalogToppingExcludeCrispy": "عدم العرض في وجبات الكرسبي",
  "admin.catalogToppingBuiltinHint":
    "الأسماء الفارغة تبقي النص من القائمة. يُحفظ السعر والصورة كتعديل للعنصر المدمج.",
  "admin.catalogToppingCrispyYes": "أيضاً في الكرسبي",
  "admin.catalogToppingCrispyNo": "برغر فقط",
  "admin.catalogToppingHiddenTitle": "إضافات مدمجة مخفية",
  "admin.catalogToppingRemoveConfirm": "إزالة هذه الإضافة من القائمة؟",
  "admin.menuExportSheetTitle": "القائمة",
  "admin.catalogExportPng": "تنزيل صورة (PNG)",
  "admin.catalogExportPdf": "تنزيل PDF (A4)",
  "admin.catalogExportBusy": "جارٍ التصدير…",
  "admin.catalogExportHint":
    "الملف يعكس القائمة الظاهرة في الكتالوج (اللغة كما في لوحة الإدارة). PNG مناسب لإنستغرام؛ PDF للطباعة أو الإرسال كمستند.",
  "admin.catalogExportErr": "فشل التصدير. أعيدوا المحاولة أو جرّبوا متصفحاً آخر.",
  "admin.inventoryTitle": "إدارة المخزون — أطباق وإضافات",
  "admin.inventoryHint":
    "حدّد العنصر عندما يكون متوفراً. ألغِ التحديد عند نفاده (افتراضياً: الكل متوفر). يشمل إضافات البرغر. تتبع الأقراص (أدناه) يحدّث توفر البرجر تلقائياً ويخصم عند الطلب.",
  "admin.inventoryPattySectionTitle": "مخزون الأقراص (غرام)",
  "admin.inventoryPattyHint":
    "بعد الحفظ، البرجر الذي لا يُحضَّر من الكميات يُعلَم غير متوفر تلقائياً؛ كل طلب يخصم الأقراص. إيقاف التتبع يلغي الخصم والحساب التلقائي.",
  "admin.inventoryPattyGramLabel": "أقراص {g} غ",
  "admin.inventoryPattySave": "حفظ كميات الأقراص",
  "admin.inventoryPattyDisableTracking": "إيقاف تتبع الأقراص",
  "admin.inventoryPattyAutoBadge": "غير متوفر تلقائياً — مخزون الأقراص",
  "admin.inventoryBurgerToppings": "إضافات البرغر",
  "admin.inventorySalads": "سلطات (مخزون)",
  "admin.inventorySaving": "جارٍ حفظ المخزون…",
  "admin.inventoryErr": "تعذّر حفظ المخزون.",
  "admin.hoursTitle": "أوقات عمل المطعم",
  "admin.hoursHint":
    "فعّل «مفتوح» لكل يوم يعمل فيه المطعم، وحدّد وقت الفتح والإغلاق. يُسمح بتجاوز منتصف الليل، مثلاً فتح 22:00 وإغلاق 02:00 — الإغلاق صباح اليوم التالي. سيظهر ذلك في نافذة «ساعات العمل» في الموقع.",
  "admin.hoursOpen": "مفتوح",
  "admin.hoursClosedReasonPlaceholder":
    "سبب الإغلاق (يظهر للزبائن تحت الرسالة في الصفحة الرئيسية)",
  "admin.hoursOpening": "وقت الفتح",
  "admin.hoursClosing": "وقت الإغلاق",
  "admin.hoursSave": "حفظ الأوقات",
  "admin.hoursSaving": "جارٍ الحفظ…",
  "admin.hoursSaved": "تم حفظ الأوقات.",
  "admin.hoursErr": "تعذّر حفظ أوقات العمل.",
  "admin.hoursErrInvalidTime":
    "وقت الفتح أو الإغلاق غير صالح. تأكّدوا أن كل يوم مفعّل فيه أوقات كاملة.",
  "admin.hoursErrCloseBeforeOpen":
    "لا يمكن أن يتطابق وقت الفتح والإغلاق. تجاوز منتصف الليل مسموح (مثلاً 22:00–02:00).",
  "admin.hoursErrInvalidDays": "بيانات الأيام غير صالحة. حدّثوا الصفحة وحاولوا مجدداً.",
  "admin.hoursErrStorage":
    "تعذّر الحفظ على الخادم (التخزين). في Vercel تأكّدوا من ربط KV وأن KV_REST_API_TOKEN رمز كامل وليس للقراءة فقط.",
  "admin.discountTitle": "العروض والخصومات",
  "admin.discountHint":
    "حدّدوا نسبة خصم للطلبات التي تتجاوز مبلغاً معيناً. الخصم يُطبّق على تكلفة الطعام فقط (بدون التوصيل)، ويمكن تشغيله أو إيقافه في أي وقت.",
  "admin.discountEnabled": "تفعيل الخصم",
  "admin.discountPercent": "نسبة الخصم (%)",
  "admin.discountMinOrder": "ابتداءً من مبلغ طلب (₪)",
  "admin.discountReason": "سبب الخصم",
  "admin.discountReasonPh": "مثلاً: عرض نهاية أسبوع / افتتاح",
  "admin.couponEnabled": "تفعيل كوبونات للزبائن",
  "admin.couponPercent": "نسبة نقاط الكوبون (%)",
  "admin.discountSave": "حفظ الخصم",
  "admin.discountSaving": "جارٍ حفظ الخصم…",
  "admin.discountSaved": "تم حفظ الخصم.",
  "admin.discountErr": "تعذّر حفظ الخصم.",
  "admin.deliveryFeesTitle": "أجور التوصيل حسب القرية",
  "admin.deliveryFeesHint":
    "حدّدوا سعر التوصيل لكل بلدة مرسومة على الخريطة. السعر يسري فقط على نقطة داخل حدود تلك البلدة.",
  "admin.deliveryFeesSave": "حفظ أسعار التوصيل",
  "admin.deliveryFeesSaving": "جارٍ حفظ أسعار التوصيل…",
  "admin.deliveryFeesSaved": "تم حفظ أسعار التوصيل.",
  "admin.deliveryFeesErr": "تعذّر حفظ أسعار التوصيل.",
  "admin.deliveryFeesErrInvalid": "يجب أن يكون سعر التوصيل رقماً بين 0 و500.",
  "admin.couponsTitle": "الكوبونات",
  "admin.couponsHint": "هنا يمكنك مشاهدة جميع الكوبونات التي أنشأها النظام وحذفها عند الحاجة.",
  "admin.couponTotalsUnused":
    "إجمالي قيمة الخصم — كوبونات غير مستخدمة (سارية):",
  "admin.couponTotalsUsed": "إجمالي قيمة الخصم — كوبونات مستخدمة:",
  "admin.couponBadgeLegend": "دليل الألوان:",
  "admin.couponFilterLabel": "تصفية حسب الحالة:",
  "admin.couponFilterEmpty": "لا توجد كوبونات بهذه الحالة.",
  "admin.couponsEmpty": "لا توجد كوبونات بعد.",
  "admin.couponCode": "رمز الكوبون",
  "admin.couponValue": "قيمة الخصم",
  "admin.couponExpiryStatus": "الصلاحية",
  "admin.couponUsedStatus": "الحالة",
  "admin.couponValid": "ساري",
  "admin.couponExpired": "منتهي",
  "admin.couponUsed": "مستخدم",
  "admin.couponNotUsed": "غير مستخدم",
  "admin.couponCreatedAt": "تاريخ الإنشاء",
  "admin.couponExpiresAt": "ينتهي في",
  "admin.couponRewardFromOrder": "مُحتسب من طلب رقم:",
  "admin.couponRedeemedTitle": "استخدام الكوبون",
  "admin.couponUsedAt": "تاريخ ووقت الاستخدام",
  "admin.couponUsedAtUnknown": "لم يُحفظ تاريخ الاستخدام",
  "admin.couponRedeemedOrderNumber": "رقم الطلب (عند الاستخدام)",
  "admin.couponRedeemedOrderUnknown": "غير محفوظ",
  "admin.couponDelete": "حذف الكوبون",
  "admin.couponDeleteConfirm": "هل تريد حذف هذا الكوبون من النظام؟",
  "admin.couponDeleted": "تم حذف الكوبون.",
  "admin.couponDeleteErr": "تعذّر حذف الكوبون.",
  "admin.promoTitle": "إعلان وسائط",
  "admin.promoHint":
    "فيديو: ارفع ملف فيديو بجودة أصلية. عند ربط Blob بالمشروع لا تبقى مقيّداً بحد ~4MB الخاص بدوال Vercel. عند الدخول للموقع يُعرض بملء الشاشة؛ يمكن للعميل الضغط «تخطي». بعد التخطي في نفس الجلسة لن يظهر مرة أخرى؛ فيديو جديد يُعرض تلقائياً. صور: شريط أفقي في الصفحة الرئيسية بين برغر 160 وبرغر 200 (تمرير من اليسار لليمين).",
  "admin.promoVideoSection": "فيديو عند الدخول",
  "admin.sliderSection": "شريط صور في الصفحة الرئيسية",
  "admin.sliderShowOnHome": "عرض الشريط في الصفحة الرئيسية للعملاء",
  "admin.sliderShowOnHomeHint":
    "عند الإلغاء لا تُحمّل الصور من التخزين (توفير Blob). تبقى القائمة أدناه للإدارة ويمكن التفعيل لاحقاً.",
  "admin.sliderShowOnHomeHintFs":
    "عند الإيقاف يُخفى الشريط عن العملاء. الصور نفسها تبقى في مجلد public/home-slider/ في المشروع.",
  "admin.sliderDisplaySaved": "تم حفظ إعداد العرض.",
  "admin.sliderHintFs":
    "يُعرض بين برغر 160 وبرغر 200. ضعوا حتى 24 صورة (jpg، png، webp، gif، avif) في مجلد public/home-slider/، ادفعوا إلى Git وانشروا — ستظهر تلقائياً للجميع دون Blob. يمكن أيضاً الرفع من الهاتف من القسم أدناه (يُحفظ في KV أو محلياً).",
  "admin.sliderKvHint":
    "الرفع من الهاتف: عند اختيار الصورة يبدأ الرفع مباشرة (JSON مضغوط — أكثر ثباتاً على الجوال). إذا فشل — اضغطوا «رفع للشريط من الهاتف» أو استخدموا الرابط أدناه. يُحفظ الرمز في localStorage. في الإنتاج يلزم Redis/KV.",
  "admin.sliderUploadLitePageLink":
    "صفحة رفع مبسّطة للجوال (إذا أعاد المتصفح التحميل بعد الكاميرا)",
  "admin.sliderUploadLiteTitle": "رفع صورة للشريط (صفحة خفيفة)",
  "admin.sliderUploadLiteHint":
    "صفحة خفيفة — اختر الصورة ثم اضغط «رفع». لا يوجد رفع تلقائي. يُحمّل الرمز من الذاكرة إن حفظتموه عبر «تحميل» في لوحة الإدارة.",
  "admin.sliderUploadLiteBack": "← العودة إلى لوحة الإدارة",
  "admin.sliderUploadLiteNeedSecret":
    "أدخلوا رمز المدير أو حمّلوا الجلسة أولاً من اللوحة الرئيسية.",
  "admin.sliderUploadPhoneBtn": "رفع للشريط من الهاتف",
  "admin.sliderUploadingKv": "جارٍ الرفع…",
  "admin.sliderUploadedKv": "أُضيفت الصورة إلى الشريط (رفع).",
  "admin.sliderUploadRequiresKv":
    "الرفع من الهاتف يتطلب Redis/KV في الإنتاج. فعّلوا Vercel KV في المشروع أو أضيفوا صوراً عبر public/home-slider/ وGit.",
  "admin.sliderUploadTooLarge": "الملف كبير جداً. جرّبوا صورة أصغر.",
  "admin.sliderMaxImages": "وصلتم إلى الحد الأقصى لصور الشريط.",
  "admin.sliderUploadInvalidImage": "الملف ليس صورة صالحة.",
  "admin.sliderUploadFailed": "فشل الرفع. حاولوا مرة أخرى.",
  "admin.sliderItemKindUpload": "صورة مرفوعة (الخادم)",
  "admin.sliderItemKindFs": "صورة من المشروع (Git)",
  "admin.sliderDeleteKv": "حذف صورة مرفوعة",
  "admin.sliderKvDeleteConfirm": "حذف هذه الصورة من الشريط؟ (لا يمكن الاسترجاع)",
  "admin.sliderKvDeleted": "حُذفت الصورة.",
  "admin.sliderEmptyFs":
    "لا توجد ملفات في مجلد public/home-slider/. أضيفوا صوراً هناك وانشروا إصداراً.",
  "admin.sliderUploadBtn": "رفع صورة للشريط",
  "admin.sliderUploading": "جارٍ رفع الصورة…",
  "admin.sliderUploaded": "أُضيفت الصورة إلى الشريط.",
  "admin.sliderEmpty": "لا توجد صور في الشريط.",
  "admin.sliderDelete": "إزالة من الشريط",
  "admin.sliderDeleteConfirm": "إزالة هذه الصورة من الشريط؟ (يُحذف الملف من Blob إن أمكن)",
  "admin.sliderDeleted": "أُزيلت الصورة من الشريط.",
  "admin.sliderDeleteErr": "تعذّرت إزالة الصورة.",
  "admin.sliderClearAll": "إزالة كل الصور",
  "admin.sliderClearAllConfirm":
    "إزالة كل صور الشريط دفعة واحدة؟ (تُحذف ملفات Blob من التخزين إن أمكن)",
  "admin.sliderClearedAll": "أُزيلت كل صور الشريط.",
  "admin.sliderClearAllErr": "تعذّرت إزالة كل الصور.",
  "admin.sliderPersistErr":
    "فشل الحفظ على الخادم (غالباً اتصال KV/Redis). تحققوا من متغيرات البيئة؛ بدون حفظ صحيح لن تُحفظ التغييرات.",
  "admin.sliderErrMissing": "اختر ملف صورة قبل الرفع.",
  "home.sliderAria": "صور إعلانية",
  "admin.promoStatusOn": "يُعرض للعملاء (عند وجود ملف)",
  "admin.promoStatusOff": "معطّل — لا يُعرض للعملاء",
  "admin.promoNoFile": "لا يوجد ملف فيديو. اختر ملفاً وارفعه.",
  "admin.promoUploadBtn": "رفع الفيديو",
  "admin.promoUploading": "جارٍ الرفع…",
  "admin.promoUploaded": "تم رفع الفيديو وتفعيله.",
  "admin.promoHomeDisplaySection": "العرض في الصفحة الرئيسية",
  "admin.promoShowOnHomeCheckbox": "عرض فيديو الإعلان في الصفحة الرئيسية (ملء الشاشة تلقائياً)",
  "admin.promoShowOnHomeHint":
    "ألغِ التحديد لإيقاف عرض الفيديو للعملاء. يبقى الملف على الخادم ويمكن إعادة التفعيل لاحقاً.",
  "admin.promoSaving": "جارٍ الحفظ…",
  "admin.promoSaved": "تم حفظ الإعداد.",
  "admin.promoErrUpload": "فشل الرفع.",
  "admin.promoErrInvalidType": "يجب رفع ملف فيديو فقط.",
  "admin.promoErrTooLarge": "الملف كبير جداً (الحد نحو 80MB).",
  "admin.promoErrTooLargeVercel":
    "الملف كبير جداً للرفع من لوحة الإدارة في الإنتاج (Vercel، غالباً حتى ~4MB). قم بضغط الفيديو أو استخدم تخزيناً خارجياً.",
  "admin.promoErrBlobConfig":
    "اتصال Vercel Blob غير مفعّل (BLOB_READ_WRITE_TOKEN). اربط Blob بالمشروع في Vercel ثم أعد المحاولة.",
  "admin.promoErrBlobUpload":
    "فشل رفع Blob في الإنتاج. تحقق من ربط Blob بالمشروع الصحيح، وضبط التوكن في Production، وإعادة النشر.",
  "admin.promoErrMissing": "اختر ملفاً قبل الرفع.",
  "admin.promoErrSave": "تعذّر حفظ الملف.",
  "admin.promoLoadEmpty":
    "لم يُحمّل وضع الإعلان. أغلقوا وأدخلوا مجدداً أو حدّثوا الصفحة.",

  "weekday.0": "الأحد",
  "weekday.1": "الإثنين",
  "weekday.2": "الثلاثاء",
  "weekday.3": "الأربعاء",
  "weekday.4": "الخميس",
  "weekday.5": "الجمعة",
  "weekday.6": "السبت",
};

/** @type {Record<Locale, Record<string, string>>} */
export const STRINGS = { he: HE, ar: AR };

/**
 * @param {Locale} locale
 * @param {string} key
 */
export function t(locale, key) {
  const L = STRINGS[locale] || STRINGS.he;
  return L[key] ?? STRINGS.ar[key] ?? key;
}

/**
 * @param {Locale} locale
 */
export function getTranslator(locale) {
  return (key) => t(locale, key);
}
